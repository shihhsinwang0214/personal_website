...
<VizPlan id="w9-3-grid" title="三條恆等式 × 兩種底層：本週方法的格子圖" type="static">
...
- **圖說（.mdx 內）**：「**圖：本週方法的格子。** 三條恆等式乘上兩種速度來源。條件速度那一列只有 Eulerian 與 progressive 兩格有人——不要 teacher 的方法只能住在這裡。CTM 在 progressive 與 Eulerian 之間滑動。」
