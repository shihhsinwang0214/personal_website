...
prereqs:
  ...
  - math-euler-error
lang: "zh"
title: "實作：建立這個單元的 Toy"
category: "courses"
group: "Unit 1 · Diffusion Models"
status: "available"
updated: 2026-09-13
summary: "把整個單元的東西縮成一個幾秒就能重跑的小實驗：2D toy 與小 MLP、ε-prediction 訓練、DDPM 與 DDIM 取樣，並畫出 score field 與軌跡圖。"
demos: []
references:
...
<Objectives>
- **為什麼要先在一個 2D toy 上實作？** 因為這裡的 $p_t$、score 與 posterior mean 都可以精確算出來；網路學到了什麼、在哪些 timestep 出錯，都能直接和 ground truth 比較。
- **這個 toy 應該讓你親眼確認什麼？** 親手完成 $\epsilon$-prediction、DDPM 與 DDIM，並用三張圖檢查 learned score、$\hat x_0$ 的誤差和兩種 sampler 的軌跡。同時確認不同 parametrization 表示相同資訊，以及非零的 loss floor 來自不可消除的 posterior uncertainty，而不是程式的 bug。
</Objectives>
...
這個單元的理論講完了，但還沒有一行程式跑過。接下來要做的，是把整個單元的東西縮成一個**幾秒就能重跑**的小實驗。
...
[下載 Week 3：Gaussian diffusion 與解析 score notebook（.ipynb）](/personal_website/notes/notebooks/diffusion-models-applications/dma-w3-lab.ipynb)

**Notebook 內容。** 用可解析的 2D Gaussian mixture 實作 forward noising、精確 score 與 Tweedie posterior mean；內含固定 seed、三組可重跑視覺化與改變 mixture 重疊程度的 TODO。這是解析 toy，不依賴任何 checkpoint。
...
- $t$ 很小：$x_t$ 幾乎就是 $x_0$。看到 $x_t$ 等於看到原圖——但那**不會**告訴你當初抽到的是哪一個 $\epsilon$，因為它的貢獻被 $\sigma_t$ 壓到幾乎看不見。條件變異數接近 1，loss 下界接近 1。
- $t$ 很大：$x_t$ 幾乎就是 $\sigma_t\epsilon$ 本身，$\epsilon$ 幾乎可以直接讀出來。條件變異數接近 0，loss 下界接近 0。

$x_0$ 是 Gaussian 時這個下界算得出精確值：$\operatorname{Var}(\epsilon\mid x_t)=\bar\alpha_t$。所以那條曲線就是 $\bar\alpha_t$ 本身，而把所有 $t$ 平均起來就是那個停住的非零數字——DDPM 的 linear schedule 下 $\overline{\bar\alpha_t}\approx0.28$。

> **該看的不是 loss 停在多少，而是它隨 $t$ 長什麼形狀。**

**實作建議：** 把 loss 依 $t$ 分箱畫出來（例如每 100 個 $t$ 一箱），你會看到一條從 ~1 掉到 ~0 的曲線。這條曲線比單一個平均 loss 有用得多——它讓你看得出模型是在哪一段 $t$ 沒學好。
</Answer>
</Question>

![示意圖：折線圖，橫軸 t 從 0 到 1000、縱軸 loss 從 0 到 1.1；橘色實線是理論下界（由接近 1 掉到接近 0），藍色散點是依 t 分箱的實際訓練 loss，水平虛線標「所有 t 平均起來 ≈ 0.3」；左右各一個小插圖，t 小時 x_t 幾乎等於原圖、t 大時 x_t 幾乎是純噪聲。](./imgs/w-3-5-1.png)

**圖：loss 的下界隨 $t$ 走。** 橘線是理論下界 $\bar\alpha_t$（$t$ 小趨近 1、$t$ 大趨近 0），藍點是依 $t$ 分箱的實測值。虛線那個 $\approx 0.3$ 就是你在 log 裡看到「停住」的那個數字——**它是所有 $t$ 平均出來的，不是模型學不動。**
...
3. **把 <Ref to="dma-reverse-process"/> 的判準做成定量版。** 對步數 $\lbrace 5,10,20,50,100,1000 \rbrace$，畫 DDPM（$\eta=1$）與 DDIM（$\eta=0$）終點的 $W_2$ 曲線。**先寫下你的猜測再跑。** 這個 toy 的 score 是精確算出來的，所以照 <Ref to="dma-reverse-process"/> 的判準，你應該預期確定性那一端不太會輸；如果你量到相反的結果，先去檢查是不是 score 不夠精確。
...
<Option>$t$ 小時接近 0、$t$ 大時接近 1。</Option>
<Option correct>$t$ 小時接近 1、$t$ 大時接近 0，因為那是 $\epsilon$ 給定 $x_t$ 的條件變異數。</Option>
<Explanation>
$t$ 大時 $x_t$ 幾乎就是 $\sigma_t\epsilon$，$\epsilon$ 幾乎可以直接讀出來，條件變異數接近 0；$t$ 小時 $x_t$ 幾乎就是 $x_0$，而知道原圖完全不告訴你當初抽到哪一個 $\epsilon$，條件變異數接近 1。$x_0$ 是 Gaussian 時這條曲線就是 $\bar\alpha_t$。
</Explanation>
</QuizBlock>
...
<Explanation>
這是有限步取樣誤差會累積的機制之一，也是 <Ref to="dma-sampler-family" /> 要談「自我修正」時的背景。
</Explanation>
</QuizBlock>
...
到這裡，我們走完了 DDPM 的整條推導鏈：選擇 Gaussian forward process、建立 denoising regression、用 Tweedie 連到 score，再用 DDPM、DDIM、SDE 或 ODE 把樣本帶回 data distribution。

但這條推導也同時固定了三項設計：起點是 Gaussian、中間路徑由 noise schedule 決定，資料與 noise 的 pairing 也不能自由選擇。**這些真的是生成模型必須接受的限制，還是只有 diffusion 做出的選擇？**

下一個單元會把起點、路徑與配對分開來看。Flow matching 的出發點，就是讓這三項設計可以被獨立選擇。
</Bridge>
