...
<Objectives>
- **建立之後三個單元共用的 2D toy 與小 MLP。** 說出為什麼要在這麼小的問題上做——真實的 $p_t$、score、後驗均值全部精確算得出來，網路學得好不好不用猜。
...
<VizPlan id="w3-5-notebook" title="Diffusion 單元實作 notebook" type="notebook">
...
這個單元把「生成」翻成了 denoising 回歸，證明 denoiser 就是 score，並得到 DDPM 與 DDIM 兩種取樣器。留下來的問題有兩個：forward process 一定要是 Gaussian 嗎？DDIM 的軌跡為什麼是彎的？下一個單元從第一個問題開始。
</Bridge>
