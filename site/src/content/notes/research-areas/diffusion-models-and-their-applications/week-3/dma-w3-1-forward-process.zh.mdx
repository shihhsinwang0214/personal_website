...
<Objectives>
- **寫出 DDPM 的 forward process，並解釋其中的性質。** 說出 Markov property 與 Linear + Gaussian 各自負責什麼，以及 variance-preserving 在 preserve 什麼、其中的 schedule 又是在調什麼。
- **從單步遞迴推出 $q(x_t\mid x_0)$ 的 closed form。** 說清楚為什麼不必真的跑 $t$ 次，以及式子裡那個 $\epsilon$ 到底是什麼（不是任何單一步加進去的那一撮）。
- **了解 forward process 是一個可設計的過程。** 說出每一個設計有什麼好處、為什麼這樣設計。
</Objectives>
...
換個角度看，這一行其實是把資料和一個 noise 用 $(\sqrt{\bar\alpha_t},\ \sqrt{1-\bar\alpha_t})$ 這組係數來做**插值**（interpolation）。而「係數平方和恆為 1」是 VP 這個選擇帶來的限制，不是插值本身要求的——<Ref to="dma-linear-path"/> 會把這組係數換成 $(t,\,1-t)$ 的直線插值（linear interpolation），<Ref to="dma-unified-equation"/> 則考慮最一般的情況，寫成一個把 diffusion、flow matching 都涵蓋進去的通式（general form）。
...
在這一篇中，我們已經將 forward process 完全定下來了；我們特別設計成每一步 $q(x_t\mid x_{t-1})$ 只加一點 noise、只看上一步、而且都是 Gaussian，使得任意時刻的 $x_t$ 都能用一行公式算出來、不必真的把遞迴跑 $t$ 次；也因此，forward process 讓我們可以輕易地製造一堆 $(x_0,x_t,\epsilon,t)$ 的配對。

接下來，我們想要探討怎麼學會 reverse process——倒著走、denoise 的那個過程。我們需要先釐清「倒著走一步」$q(x_{t-1}\mid x_t)$ 到底需要知道什麼：我們會從 Bayes 定理出發，找到一個可以利用這些配對去學習的回歸目標，讓 $q(x_{t-1}\mid x_t)$ 真的抽得出樣本來。
</Bridge>
