...
<Objectives>
- **生成模型到底在學什麼？** 「生成一張新圖」不是找回某一筆資料，也不是只覆蓋幾個 mode，而是學到一個接近 $p_{\text{data}}$ 的 distribution，讓我們可以再從中抽樣。
- **為什麼我們最後會想自己指定一條路徑？** 直接學 density 會卡在 normalizing constant 與 sampling；直接學 transport map，沒有 pairing 的 $L_2$ 會塌成平均，normalizing flow 又必須用可逆性與 log-det 換取 likelihood。Diffusion 的選擇，是把原本不唯一的生成方式改成一條自己知道怎麼走的路。
</Objectives>
...
這一篇先把「生成新樣本」翻成「學會從 $p_{\text{data}}$ 再抽一個點」。直接學 density 會卡在 normalizing constant；直接學一個 transport map，沒有 pairing 的 $L_2$ 會塌成平均，而 normalizing flow 又必須用可逆性與 Jacobian 換取 likelihood。

這些困難把我們帶到另一個問題：**如果起點到資料之間的路不只一條，我們能不能自己選一條容易學的？**

DDPM 的答案，是先把方向反過來：從資料出發，一步一步加入 Gaussian noise，親手造出一條通往簡單分布的路。下一篇會把這條 forward process 定下來，並看見為什麼它的任意時刻都可以直接抽樣。
</Bridge>
