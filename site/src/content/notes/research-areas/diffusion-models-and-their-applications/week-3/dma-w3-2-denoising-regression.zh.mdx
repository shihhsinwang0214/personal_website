...
兩件事其實可以分開設計。因此，即使 forward process 不變，我們仍然可以改變不同 $t$ 在 loss 裡的 weighting。理想情況下這不改變每個 $t$ 對應的最佳 predictor，但會改變實際訓練時模型把 capacity 和 optimization effort 放在哪些地方。<Ref week="dma-week-flow-matching"/> 比較 diffusion 與 flow matching 時，這個 distinction 還會再出現。
...
這裡故意只寫成 pseudo code。真的要跑，還得決定 batch 大小、$T$、schedule、網路架構、optimizer——那些都不改變上面這五行在說什麼。<Ref to="dma-lab-diffusion"/> 會把同樣這五行寫成真的跑得動的程式；到時候會看到 loss 掉到某個值就停住不動，那不是寫錯了，<Ref to="dma-tweedie-and-score"/> 會說它為什麼停在那裡。
