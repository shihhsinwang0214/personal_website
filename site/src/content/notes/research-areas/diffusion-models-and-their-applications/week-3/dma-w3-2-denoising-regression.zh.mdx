...
<Objectives>
- **倒著走一步，怎麼變成一個可以訓練的 regression？** 真正的 $q(x_{t-1}\mid x_t)$ 算不出來，但 training 時我們知道 $x_0$。把它放進條件後，新的 objective 只和原本相差一個與模型參數無關的常數；再選擇固定變異數的 Gaussian parametrization，原本難以直接計算的 KL objective，就會化成可以輕鬆計算的 weighted MSE。
- **模型學完後，怎麼真的從 $x_T$ 走回 $x_0$？** 網路可以預測 $x_0$、$\epsilon$ 或 $v$：三者能互相轉換，但相同形式的 MSE 會給不同 timestep／SNR 不同權重。把預測結果換回 reverse Gaussian 的平均值，就能一步一步完成生成。
</Objectives>
...
我們把原本不好算的 reverse problem，變成了可以實際訓練的 weighted MSE；也看到網路可以預測 $x_0$、$\epsilon$ 或 $v$。三者表示相同資訊，但相同形式的 MSE 會給不同 timestep 不同的權重。

可是 DDPM 實際使用的 $\mathcal L_{\text{simple}}$，偏偏把理論推導出的 weighting 拿掉了。**這樣訓練出來的網路，到底在學什麼？為什麼改掉 weighting 之後仍然有效？**

下一篇會固定一個 noise level，直接找出這個 regression 的最佳解。答案是 $\mathbb E[\epsilon\mid x_t]$；而 Tweedie formula 會進一步告訴我們，這個 conditional expectation 正是 noisy distribution $p_t$ 的 score。
</Bridge>
