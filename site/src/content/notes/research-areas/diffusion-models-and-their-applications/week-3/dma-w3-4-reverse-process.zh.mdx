...
**代回我們的情形**：DDPM 的 forward 是 $f_t=-\tfrac12\beta(t)$、$g_t=\sqrt{\beta(t)}$，所以 $g_t^2=\beta(t)$——代進去就字字對得上上面那兩條。認得通式的好處是：**換一個 schedule、甚至換一整族 forward process，變的只有 $f_t$ 與 $g_t$，這兩條式子的形狀不變。** <Ref to="dma-unified-equation" /> 會把這些 choices 放進 stochastic interpolant 的共同架構。
</Remark>
...
這件事之後會被推廣成一整族取樣器（<Ref to="dma-sampler-family" />）：噪聲要注入多少，其實是一個**可以調的設定**。
</Remark>
