...
status: "available"
updated: 2026-09-06
summary: "曲率積分壓小了誤差，但只要它不是零，一步 Euler 就有誤差。這個單元換一個目標：不學速度場，直接學把噪聲送到資料的那個映射。先看最笨的做法——用 teacher 跑 ODE 造配對、直接回歸終點——哪裡好、哪裡不好，發現它和 reflow 是同一個操作；再用半頁把時間方向切換到本單元的慣例。"
demos: []
references:
...
- 把時間方向從 <Ref week="dma-week-flow-matching"/>、<Ref week="dma-week-interpolants"/> 的 FM 慣例切換到本單元的慣例（$t=0$ 是資料、$t$ 大是噪聲），並能在兩套座標之間來回翻譯。
</Objectives>
...
換句話說，前三個單元的做法都是**先學速度場、再用數值方法積分它**，「一步生成」在這條路上是一個永遠只能逼近的極限。這個單元換一個問題：我們真正想要的是「從噪聲一步到資料」的那個映射。
