...
status: "available"
updated: 2026-09-07
summary: "absorbing 鏈翻開的 token 永遠固定，uniform 鏈每一步都能改任何 token。前者像 ODE：沒有修正、誤差累積；後者像 SDE：有自我修正、但代價是模型分不清哪些字是噪聲。只給直覺與實驗，數學留到下一個單元的 rate 語言。"
demos: []
references:
...
- 說出為什麼實務上 absorbing 通常表現更好，儘管它不能修正——並由此看見下一個單元 remasking 要解決的問題。
</Objectives>
...
下一個單元換一種語言：連續時間，用 **rate** 描述「每單位時間有多少機率從一個狀態流到另一個狀態」。在那個語言裡，remasking 只是在 absorbing 過程上多加一個 rate，而它對應到的正是 <Ref to="dma-sampler-family"/> 那個「訓練噪聲與取樣噪聲是兩個旋鈕」裡取樣器的 $\varepsilon$。答案會變得很短。
</Answer>
</Question>
