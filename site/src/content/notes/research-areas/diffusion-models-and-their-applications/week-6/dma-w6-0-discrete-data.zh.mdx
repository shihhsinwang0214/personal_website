...
status: "available"
updated: 2026-09-07
summary: "文字、蛋白質序列、分子圖都是 token，沒有「加一點 Gaussian noise」這件事。盤點前三週哪些環節壞了、哪些還活著，把「加噪聲」換成一條已知的 Markov chain，並釐清：discrete 指的是狀態空間，不是時間。"
demos: []
references:
...
<Objectives>
- 說出 <Ref week="dma-week-diffusion"/> 的框架裡，哪些環節依賴「狀態是連續的」、哪些只依賴「forward process 是已知的 Markov chain」。
- 把「加噪聲」在離散狀態空間上換成一條合法的 forward chain，並說出兩個標準選擇（uniform 與 absorbing / mask）各自的終點分佈。
- 分清「discrete」指的是**狀態空間**離散，不是時間離散，並說出為什麼這件事容易混。
</Objectives>
...
前三週的資料一直是一堆 $\mathbb R^d$ 裡的點：一張圖是 3072 個實數，一個 toy 樣本是平面上的一個座標。這一週把資料換掉。
...
於是 <Ref week="dma-week-diffusion"/> 的第一步就出問題了。那一週的起點是「把 $x_0$ 一點一點加 Gaussian noise，直到變成 $\mathcal N(0,I)$」。對一串 token，$x_0+\sigma\epsilon$ 根本不是一個合法的物件。

先別急著找替代品。值得先做的是**盤點**：前三週的框架裡，哪些東西真的用到了「狀態是連續的」，哪些只是用到了「forward process 是一條我們自己寫好的、邊際好算的鏈」。盤點完會發現，壞掉的比想像中少。
...
所以這一週的任務比重新發明一個框架小得多：**把「加噪聲」換成一條離散的 Markov chain，然後看回歸目標長什麼樣。**
...
「embedding 加 Gaussian noise 再 round」是第三條路（Diffusion-LM [3] 走過），它把問題丟回連續空間，於是前三週的工具都能用。代價是 round 這一步：噪聲小的時候 round 回原字、噪聲大的時候 round 到隨機字，中間幾乎沒有「介於兩個字之間」的有意義狀態，模型花大量容量在學 embedding 的幾何而不是語言本身。在語言上，mask 的表現長期壓過這條路。這門課接下來只走前兩條。
</Answer>
</Question>

## 一個容易混的詞

到這裡要停下來釐清一個詞。這一週的標題是 **discrete diffusion**，「discrete」說的是**狀態空間**是離散的：$x_t$ 的每個位置是 $K$ 個類別之一。

它**不是**在說時間離散。<Ref week="dma-week-diffusion"/> 的 DDPM 走 $T=1000$ 步，時間也是離散的，但每個 $x_t$ 是 $\mathbb R^d$ 裡的點，狀態連續。反過來，<Ref week="dma-week-discrete-ii"/> 會把這一週的 Markov chain 換成連續時間的版本——時間連續，狀態依然離散。四種組合都存在：

| | 狀態連續 | 狀態離散 |
|---|---|---|
| **時間離散** | DDPM（<Ref to="dma-reverse-process"/>） | D3PM、masked diffusion（本週） |
| **時間連續** | SDE / ODE（<Ref to="dma-reverse-process"/>、<Ref to="dma-sampler-family"/>） | CTMC、SEDD、discrete FM（<Ref week="dma-week-discrete-ii"/>） |

這張表值得記住，因為它同時說明了本週和下週的分工：本週在左下→右上這條對角線上做翻譯——把時間離散、狀態連續的 DDPM 翻成時間離散、狀態離散的版本，讓你先在熟悉的結構裡接受「離散噪聲」；下週再把時間變連續，重建 score 與 sampler 的語言。
...
代價也有，而且和 <Ref to="dma-sampler-family"/> 那個「ODE 沒有自我修正」的觀察是同一件事：被遮的 token 一旦被還原，就**永遠固定**，之後的步驟沒有機會改它。這一週先只給直覺，<Ref week="dma-week-discrete-ii"/> 有了 rate 的語言之後會把它量化。

<VizPlan id="w6-0-two-chains" title="兩條鏈，兩個終點" type="static">
- 檔名 `imgs/w6-0-1.png`，放在「為什麼 mask 值得特別看」之後、「先消化一下」之前。
- 版面：上下兩條「時間軸」，橫軸都是 $t$（左 $t=0$、右 $t=T$），每條軸上放五個時刻的十二格序列縮圖。
  - **上「uniform」**：格子從全部橘（原字）逐漸混入藍格（隨機字），最後整排都是藍的、每格一個不同的隨機字。右端一個小方框寫「終點：每個位置獨立均勻」。
  - **下「absorbing」**：格子從全部橘逐漸變灰（`[MASK]`），最後整排全灰。右端小方框寫「終點：整串 `[MASK]`（一個確定的狀態）」。
- 兩條軸中間畫一條共用的 $\bar\alpha_t$ 曲線（從 1 掉到 0），並用引導線標出「$\bar\alpha_t$＝還是原字的機率，兩條鏈共用同一個意思」。
- 下方一個小框對照 diffusion：`√ᾱ_t x_0 + σ_t ε` 與 `ᾱ_t·原字 + (1−ᾱ_t)·終點`，標「同一個角色：還剩多少原始訊號」。
- 底部一句：「兩條鏈都是我們自己寫好的、每一步規則已知的 Markov chain；差別只在終點，以及被動過的 token 還能不能回來。」
- 風格：與 `w-3-0-2.png` 一致（淺底、扁平、繁體中文標註、原字橘 `#f0803c`／隨機字藍 `#4a90e2`／`[MASK]` 灰）。
- 長寬比：約 16:9。
</VizPlan>
...
<Explanation>
DDPM 走 $T$ 步、時間也是離散的，但狀態在 $\mathbb R^d$ 裡連續。discrete diffusion 說的是狀態離散；時間可以離散（本週）也可以連續（下週）。
</Explanation>
</QuizBlock>
...
<Explanation>
已知路徑只需要一條 Markov chain；回歸只需要一個可以最小化的 loss（MSE 或 cross-entropy）。只有 score 與 ODE 需要 gradient，而類別之間沒有方向。這兩塊要等下週用 rate 與比值重建。
</Explanation>
</QuizBlock>
...
<Bridge to="dma-d3pm">
本篇把「加噪聲」換成一條已知的 Markov chain，並選定 uniform 與 absorbing 兩條標準路徑。下一篇把它們寫成 transition matrix，算出 $q(x_t\mid x_0)$ 的 closed form，並看 <Ref to="dma-denoising-regression"/> 的回歸目標在類別世界裡長什麼樣。
</Bridge>
