...
status: "available"
updated: 2026-09-07
summary: "文字、蛋白質序列、分子圖都是 token，沒有「加一點 Gaussian noise」這件事。盤點前三個單元哪些環節壞了、哪些還活著，把「加噪聲」換成一條已知的 Markov chain，並釐清：discrete 指的是狀態空間，不是時間。"
demos: []
references:
...
先別急著找替代品。值得先做的是**盤點**：前三個單元的框架裡，哪些東西真的用到了「狀態是連續的」，哪些只是用到了「forward process 是一條我們自己寫好的、邊際好算的鏈」。盤點完會發現，壞掉的比想像中少。
