...
status: "available"
updated: 2026-09-07
summary: "網路輸出的是逐 token 獨立的 marginal，一步同時翻開多個 token 就等於假設它們條件獨立。用 parity toy 精確算出這個誤差、寫成 KL，並指出它與連續世界「曲率造成有限步誤差」是同一個角色；與 autoregressive 模型的取捨也由此而來。"
demos: []
references:
...
<Objectives>
- 說出「一步同時翻開多個 token」到底假設了什麼，並在 parity toy 上精確算出它造成的錯誤率。
- 把這個誤差寫成一個 KL（被翻開的那組位置的 conditional joint 與 marginal 乘積之間），並說出它何時為零。
- 對照 <Ref to="dma-error-theory"/>：連續世界有限步誤差的來源是曲率，離散世界的是因子化假設；並由此說出 masked diffusion 與 autoregressive 模型的取捨。
</Objectives>
...
值得注意的是這個誤差**不在訓練 loss 裡**。<Ref to="dma-masked-diffusion"/> 的 ELBO 是對「每步翻開一個位置」的鏈算的（$T\to\infty$ 時每步幾乎只翻開零或一個位置），它的最小值是完美的 marginal。因子化誤差純粹是取樣時「一步走太大」造成的——這和 <Ref to="dma-error-theory"/> 的情形一模一樣：訓練目標是精確的速度場，Euler 誤差是離散化造成的。
</Details>
...
| | 連續（<Ref week="dma-week-interpolants"/>） | 離散（本週） |
|---|---|---|
...
這張表是這一週最重要的一張。它說的是：**有限步取樣的誤差，在兩個世界裡都來自「一步之內假設了太簡單的結構」。** 連續世界假設直線，離散世界假設獨立。<Ref to="dma-rectified-flow"/>、<Ref to="dma-minibatch-ot"/> 在連續世界做的「拉直」，在離散世界沒有直接對應——沒有「直的配對」這回事——所以離散世界減少誤差的手段目前主要落在最後一行的後兩項：MaskGIT [3] 用信心排序挑位置；近期的工作（例如 Discrete Copula Diffusion [4]）則試著在取樣時補回相依的結構。這條線還在發展中。
...
<VizPlan id="w6-3-two-worlds" title="一步之內假設了太簡單的結構" type="static">
- 檔名 `imgs/w6-3-1.png`，放在「與 autoregressive 的取捨」之後、「先消化一下」之前。
- 版面：左右兩塊，各是一個世界，中間一條垂直虛線分隔，上方橫跨兩塊寫一句大標「有限步的誤差，都來自一步之內假設了太簡單的結構」。
  - **左「連續」**：一條彎曲的軌跡（綠），上面疊一條 Euler 的割線（靛），兩者之間的落差用陰影標出；下方寫「假設：這一步內是**直線**」與「誤差：$\int\|\ddot x_t\|\,dt$」。
  - **右「離散」**：兩個相鄰的 `[MASK]` 格子，左邊畫真實 joint 的 2×2 表（兩格有值、兩格灰），右邊畫 marginal 乘積的 2×2 表（四格都有值），兩表之間畫一個「≠」；下方寫「假設：這一步翻開的位置**條件獨立**」與「誤差：$\mathrm{KL}$（joint ‖ 乘積）」。
- 兩塊下方各一行「何時為零」：左「軌跡是直線」、右「每步只翻開一個位置（＝autoregressive）」。
- 底部一句：「連續世界假設直線，離散世界假設獨立；兩邊都是多走幾步就會變好，但把假設本身變便宜的手段不一樣。」
- 風格：與 `w-3-0-2.png` 一致（淺底、扁平、繁體中文標註、路徑綠 `#60ba82`／強調靛 `#2a4ea3`／不合法的格子用灰）。
- 長寬比：約 16:8。
</VizPlan>
...
本篇把「一步翻開太多」的代價寫成 total correlation，並把它放到與曲率同一個位置上。還有一個上一篇就埋下的問題沒有處理：翻錯的字能不能改？absorbing 說不能，uniform 說可以——下一篇比較這兩條鏈在「修正」上的差別，並把它連回 <Ref to="dma-sampler-family"/> 的 ODE 與 SDE。
</Bridge>
