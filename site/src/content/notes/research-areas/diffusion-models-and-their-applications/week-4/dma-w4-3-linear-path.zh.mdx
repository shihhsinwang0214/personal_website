...
status: "available"
updated: 2026-09-13
summary: "取 α_t=t、σ_t=1−t，條件速度就是「終點減起點」；換到 ε 座標會看到它其實帶了 1/t² 的加權。條件路徑是直的，邊際軌跡卻是彎的——彎來自交叉，交叉來自配對。"
demos: []
references:
...
<Objectives>
- **寫出線性條件路徑的訓練目標，並說出程式碼要改的是哪一行。** 條件速度是常數 $x_1-x_0$，`target` 那一行而已。
- **說出邊際軌跡為什麼是彎的，並把「彎」一路追到「配對方式」。** 用兩組配對就能把交叉點上的平均速度算出來，而那個方向沒有任何一條條件直線在走。
- **把 FM 的 loss 換到 $\epsilon$ 座標，看出它隱含一個 $1/t^2$ 的加權。** 並說出它和 DDPM 的 $\mathcal L_{\text{simple}}$（均勻加權）差的就是這個權重。
</Objectives>
...
理解它為什麼不會，是 <Ref week="dma-week-interpolants"/> 整週的起點。一句話串起來：**「彎」來自「交叉」，「交叉」來自「隨機配對」。** 這三個詞在 <Ref week="dma-week-interpolants"/> 分別長成 <Ref to="dma-error-theory"/>、<Ref to="dma-rectified-flow"/> 與 <Ref to="dma-minibatch-ot"/>。
</Answer>
</Question>
...
第一，**學速度和學噪聲仍然是同一件事**，兩者只差一個 affine transformation——這延續 <Ref to="dma-denoising-regression"/> 對三種 parametrization 的討論。（那裡把「預測資料」叫 $x_0$-prediction，因為 diffusion 的慣例裡 $x_0$ 是資料；在本週的慣例下同一件事要叫 $x_1$-prediction。**這是最容易讀錯的一個術語。**）
...
<Ref to="dma-denoising-regression"/> 說過「路徑」與「加權」是兩個獨立的設計軸，這裡是它最乾淨的例子：兩個模型可以只差加權，訓出來的結果卻不一樣。<Ref to="dma-fm-vs-diffusion"/> 會把這兩軸攤在同一張表上。

<VizPlan id="w4-3-crossing-static" title="交叉點上的平均：一張圖" type="static">
- 檔名 `imgs/w4-3-1.png`，放在「直不直，決定要走幾步」之後、「消化一下」之前。
- 版面：左右兩塊，共用同一組四個團（左下、右下為藍色起點；左上、右上為橘色資料）。
  - **左「隨機配對」**：兩條綠色直線交叉成一個 X，交叉點畫一個小白圈；從白圈出發畫兩支細綠箭頭 $(2,2)$ 與 $(-2,2)$，再疊一支垂直向上的粗靛藍箭頭標 $u_t=(0,2)$；旁註「水平分量抵消，剩下往上」。再用兩條靛藍曲線畫實際的邊際軌跡：先向中間靠攏、再分岔到左上與右上，明顯是彎的。
  - **右「依位置配對」**：左下→左上、右下→右上兩條平行的綠色直線，完全不交叉；邊際軌跡與條件直線重合，兩條直的靛藍線。
- 兩塊下方各標一個數字：左「彎曲度 1.59 · 4 步誤差 1.49」、右「彎曲度 1.00 · 4 步誤差 0.03」。
- 底部一句：「兩邊的 $p_0$ 與 $p_1$ 完全相同；差別只在誰配誰。」
...
FM 的標準形式有了，軌跡為什麼彎也知道了。下一篇把 FM 和 <Ref week="dma-week-diffusion"/> 的 diffusion 正式放在一起：哪些真的是同一件事、哪些只是看起來像。
</Bridge>
