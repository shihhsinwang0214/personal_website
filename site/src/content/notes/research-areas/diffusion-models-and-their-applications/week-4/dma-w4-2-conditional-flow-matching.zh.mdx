...
status: "available"
updated: 2026-09-13
summary: "邊際速度場寫不出來，但條件速度場好算到不需要網路。把難算的目標換成好算的條件目標，回歸會自己取後驗平均——這跟 Tweedie 那一篇是同一招。"
demos: []
references:
...
<Objectives>
- **說出邊際速度場 $u_t$ 為什麼寫不出來，並認出這是 <Ref to="dma-denoising-regression"/> 那個 conditional trick 的速度版本。** KL 版本與 MSE 版本已經各走過一遍。
- 用「先抽一組端點對、再把路線寫死」造出一個好算的回歸目標。
- 說出 CFM 與 FM 的關係：不只最小值相同，梯度處處相同——所以我們從頭到尾都不用把那個平均算出來。
</Objectives>
...
麻煩就出現在 $u_t(x)$ 這一項——那是「在位置 $x$、時刻 $t$ 的正確速度」，本篇下一節會把它精確定義出來。它不是我們手上有的東西：要知道 $x$ 這個位置的正確速度，得先知道整條分布路徑 $p_t$ 長什麼樣，而 $p_t$ 又要有整個 $p_{\text{data}}$ 才定得下來。我們有的只是**五萬個資料點**。
...
（表裡前兩欄用的是 <Ref week="dma-week-diffusion"/> 的慣例——那裡的 $x_0$ 是**資料**；最右欄是本週的慣例，配對寫成 $z=(x_0,x_1)$，$x_0$ 是**噪聲**。）
...
<Question id="Q2">
$x_0$ 和 $x_1$ 是**各自獨立**抽的，所以配對是隨機的：一個角落的噪聲點很可能被配到對面角落的資料點，路線因此交錯成一團。

**這樣學出來的東西還會是對的嗎？**

<Answer>
分布是對的。定理 1 和定理 2 的證明從頭到尾沒有用到「路線不交叉」，也沒有用到 $x_0$ 與 $x_1$ 獨立——只用到條件路徑的兩個端點對、以及條件期望的性質。所以 $p_1=p_{\text{data}}$ 這件事不受影響。

但**軌跡**會付出代價。網路在一個位置只能輸出**一個**速度，而交錯的地方有很多條路線經過、方向各不相同，於是它輸出的是那些方向的平均。demo 裡那支明顯比細箭頭短的粗箭頭就是這件事：平均之後不但變短，也不再對齊任何一條原本的直線。

後果是實際軌跡是**彎的**。彎的軌跡在步數多的時候沒差，步數一少，離散化誤差就跑出來——<Ref to="dma-linear-path"/> 會把這件事算清楚。

順著這個念頭，一個很自然的問題是：既然配對是我們自己挑的，能不能挑一組**不那麼交錯**的？可以，而且這正是 <Ref to="dma-rectified-flow"/> 與 <Ref to="dma-minibatch-ot"/> 在做的事。
</Answer>
</Question>

<VizPlan id="w4-2-condition-then-average" title="條件到邊際：一張圖說完三步" type="static">
- 檔名 `imgs/w4-2-1.png`，放在 Q2 之後、「消化一下」之前。
- 版面：三格從左到右，箭頭串起來（步驟 1 → 2 → 3）。
  - **左「抽一組配對」**：左側一團藍色噪聲點、右側一團橘色資料點，其中**一個藍點與一個橘點**被連起來、其餘淡化；標註 $z=(x_0,x_1)$。
  - **中「路線寫死、速度已知」**：同一對點之間一條綠色直線，線上標幾個時間刻度 $t=0,0.5,1$，並在中點畫一支綠色速度箭頭；旁註「給定配對，速度是兩個純量乘兩個向量——不需要網路」。
  - **右「平均成邊際」**：畫二十來條配對直線都經過同一個位置 $x$（用一個小白圈標出），從 $x$ 出發畫出這些線各自的細綠箭頭（方向散開），再疊一支明顯較短的粗靛藍箭頭標 $u_t(x)$；旁註「網路在這裡只能輸出一個速度：它們的加權平均」。
- 底部一句：「難算的是右邊那一格；但我們只需要訓練時每次餵中間那一格，$L_2$ 回歸會自己走到右邊。」
...
框架建好了，而且可以代任何一條 $(\alpha_t,\sigma_t)$ 進去。下一篇代入最簡單的那一條——直線——然後問一個看起來理所當然、答案卻是「不會」的問題。
</Bridge>
