...
status: "available"
updated: 2026-09-13
summary: "Gaussian 路徑上速度場與 score 只是同一個物件的兩套座標，寫出精確的轉換式並代 VP 驗一次；再從四個旋鈕（起點、路徑、配對、加權）把兩個框架的差異分開來看，likelihood 與取樣器則是後果。"
demos: []
references:
...
<Objectives>
- **寫出 Gaussian 路徑下速度場與 score 的精確轉換式，並代 VP 進去驗出它就是 PF-ODE。** 兩個係數各化簡一次，都會變成 $-\beta/2$。
- **把 FM 與 diffusion 的差異分到四個旋鈕上**：<Ref to="dma-why-not-diffusion"/> 的三個 forward 旋鈕（起點／路徑／配對）加上訓練端的加權。likelihood 與取樣器不是旋鈕，是後果。
- **說出「條件路徑是直的」不等於「邊際軌跡是直的」，並知道要判斷哪一條比較好走只能量。**
</Objectives>
...
<Ref week="dma-week-diffusion"/> 和這個單元看起來差很多：一邊是 SDE、score、Tweedie，一邊是 ODE、速度場、回歸。但兩邊的中間分布都是「縮小的資料加上 Gaussian noise」，這實在太像了。
...
- <Ref to="dma-why-not-diffusion"/> 的**三個 forward 旋鈕**——起點分布、路徑、配對。它們決定 $p_t$ 長什麼樣。
- 再加一個**訓練端的旋鈕：加權**。它不改 $p_t$、也不改最佳解，只改「有限預算下先學好哪一段 $t$」。<Ref to="dma-why-not-diffusion"/> 那三個旋鈕沒有涵蓋它，因為它根本不在 forward process 裡。

下面四段就是這四個。其餘的（likelihood 怎麼算、能不能用 SDE 取樣）都是**這四個選完之後的後果**，不是可以獨立轉的旋鈕。
...
| **旋鈕**：路徑 | VP／VE，固定 | 任意 $(\alpha_t,\sigma_t)$ |
| **旋鈕**：配對 | 獨立 | 任意耦合 |
| **旋鈕**：加權 | 由 $\epsilon$-loss 與 schedule 隱含 | 由目標與 $t$ 的分布隱含 |
...
這正是上面那四個旋鈕的用處：一個實驗結果要歸因到哪一個，得把其他三個控制住才說得清。上面那個 2D 的反例不是要說線性路徑不好，而是要說**這件事不能用框架的名字來推論。**
</Details>

<VizPlan id="w4-4-one-object-two-coords" title="同一個物件的兩套座標" type="static">
- 檔名 `imgs/w4-4-1.png`，放在四個旋鈕的表格之後、Q1 之前。
- 版面：中間一個大方塊寫「$p_t$ 的同一個物件」，左右各一組符號，用雙向箭頭連到中間。
  - **左（diffusion 座標）**：$\epsilon_\theta(x,t)\ \approx\ \mathbb E[\epsilon\mid x_t]$、$\nabla\log p_t=-\epsilon_\theta/\sigma_t$；下方小字「學噪聲、走 SDE 或 PF-ODE」。
  - **右（FM 座標）**：$u_\theta(x,t)\ \approx\ \dot\alpha_t\mathbb E[x_1\mid x_t]+\dot\sigma_t\mathbb E[\epsilon\mid x_t]$；下方小字「學速度、解 ODE」。
  - 雙向箭頭上標「只是 affine transformation，不用重訓」，並把那個框起來的轉換式寫在箭頭下方。
- 右側再掛一個小標註：「這個等價**只在 Gaussian 路徑上**成立；路徑、加權、配對三個軸換掉之後就不是同一個物件了。」
- 底部一句：「兩邊學的是同一件事；差別在你允許自己換掉哪幾樣東西。」
- 風格：與 `w-3-0-2.png` 一致（淺底、扁平、繁體中文標註、噪聲藍 `#4a90e2`／資料橘 `#f0803c`／強調靛 `#2a4ea3`）；數學符號用簡單上下標，不要試著排 LaTeX。
- 長寬比：約 16:7。
</VizPlan>
...
<Explanation>
兩者在 Gaussian 路徑上是同一個 ODE，所以差異只能來自那四個旋鈕（起點、路徑、配對、加權）之一。沒有分開控制的比較，說不出是哪一個造成的。
</Explanation>
</QuizBlock>
...
理論到這裡告一段落。下一篇動手把 FM 加進 <Ref week="dma-week-diffusion"/> 的那個 toy，順便把這一篇「不用重訓就能換取樣器」的說法親手驗一次。
</Bridge>
