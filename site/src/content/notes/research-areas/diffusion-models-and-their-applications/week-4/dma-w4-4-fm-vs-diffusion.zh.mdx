...
<Ref week="dma-week-diffusion"/> 和這一週看起來差很多：一邊是 SDE、score、Tweedie，一邊是 ODE、速度場、回歸。但兩邊的中間分布都是「縮小的資料加上 Gaussian noise」，這實在太像了。
...
- 再加一個**訓練端的旋鈕：加權**。它不改 $p_t$、也不改最佳解，只改「有限預算下先學好哪一段 $t$」。W4.0 那三個旋鈕沒有涵蓋它，因為它根本不在 forward process 裡。
