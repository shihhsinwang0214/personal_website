...
本篇把 flow matching 的建構搬進離散空間：條件路徑是「每個 token 跳一次」，邊際 rate 是條件 rate 的後驗平均，absorbing 與 uniform 是 source 的兩個選擇，配對留下對齊的意義。這個單元的語言到此齊備：path、target、sampler 三個槽位。下一篇用這三個槽位去讀真實的系統——語言模型、蛋白質、圖。
</Bridge>
