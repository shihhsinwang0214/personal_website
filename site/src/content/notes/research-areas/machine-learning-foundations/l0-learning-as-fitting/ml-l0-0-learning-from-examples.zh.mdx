...
<Objectives>
- 把「從例子學規則」拆成三個可以獨立更換的零件——假設空間、損失、資料——並說出更換每一個會改變什麼。
- 寫出經驗風險最小化（ERM）的式子，說出它與「真正想最小的東西」差在哪裡，並把總誤差拆成 approximation、estimation、不可約三塊。
- 對「訓練誤差越低越好」這個直覺，判斷它在什麼條件下成立、什麼時候變成陷阱，並在系列共用的 toy 上把陷阱跑出來。
</Objectives>
...
- **資料**是「他看過什麼」。看得少、或看的和考的不是同一種房子，前兩個旋鈕調得再好也沒用。

這三個旋鈕解釋了為什麼「模型不夠好」不是一個診斷。它至少有三種病因，處方完全不同，而且**互相不能替代**：資料太少不會因為換更大的模型而好轉。

<VizPlan id="ml-l0-0-three-knobs" title="同一份資料，三個旋鈕各轉一格" type="static">
- **檔名**：`l0-learning-as-fitting/imgs/l0-0-1.svg`（16:9，2×2 格）。
- **左上（基準）**：共用 toy 的 30 個點（$y=\sin(2\pi x)+$ 噪聲），配一條 degree 3 的擬合曲線與淡灰色的真實 $\sin$ 曲線。標「$\mathcal F$ = 三次多項式、$\ell$ = 平方、$n=30$」。
- **右上（換假設空間）**：同一組點，改畫一條直線（欠擬合）與一條 degree 25 的狂震曲線（過擬合）疊在一起，兩條用不同色。標「只換 $\mathcal F$」。
- **左下（換損失）**：同一組點但加入 3 個離群值，畫平方損失與絕對損失兩條擬合，平方那條明顯被離群值拉走。標「只換 $\ell$」。
- **右下（換資料）**：只給 $x\in[0,0.6]$ 的點，擬合曲線在 $x>0.6$ 的區域外推得離譜。標「只換 $D$」。
- **圖說**：「三個零件各自壞掉的樣子不一樣——這就是為什麼『模型不夠好』不是一個診斷。」
</VizPlan>
...
再加上誰都消不掉的 $R(f^\star)$（不可約誤差）。三塊各對一個旋鈕：**空間太小 → approximation 大；空間太大、資料太少 → estimation 大；噪聲本身 → 不可約。** 把 $\mathcal F$ 放大會壓低第二項、抬高第一項——這個對衝就是下一個 class 的主題。
...
**資料。** 他只看過某一區、某一段時間的房子。「新房子與看過的房子同分佈」這個假設，在換城市、換年份、換房型時就破了——那時前兩個旋鈕調得再好都沒用。這是 <Ref to="ml-empirical-risk" mode="title"/> 的主題。
...
<iframe loading="lazy" src="/personal_website/notes/research_areas/machine-learning-foundations/l0-learning-as-fitting/l0-0-2.html" class="demo-frame" title="轉三個旋鈕，看兩條風險怎麼分家：互動 demo" style="height: 827px; width: 100%; border: none;"></iframe>
...
這一篇把「從例子學規則」拆成三個零件（假設空間、損失、資料），寫出 ERM，並把成績拆成 approximation、estimation 與不可約三塊。共用 toy 上的四行輸出示範了整個系列的核心落差：經驗風險單調下降，母體風險先降後爆，而你在訓練時只看得到前者。

三個零件裡，最常被當成「預設值」而從不檢查的是**損失**。多數人直接寫 MSE，然後才發現模型學到的東西不是自己要的。下一篇把損失當成一個要主動宣告的決定：差 1 度與差 10 度的相對嚴重程度一旦寫下，最佳答案落在哪裡就被決定了——均值、中位數、還是某個分位數。
</Bridge>
