...
有了訓練的手段，就可以回到 <Ref to="ml-mlp" mode="title"/> 留下的那個問題：MLP 對「哪些像素相鄰」一無所知（打亂像素順序，測試準確率從 $0.968$ 變成 $0.975$），而且換一個位置就不會了（$0.625$）。接著把「局部」與「平移之後還是同一件事」寫進假設空間，並在同一個 toy 上量出那兩個數字各變成多少。
</Bridge>
