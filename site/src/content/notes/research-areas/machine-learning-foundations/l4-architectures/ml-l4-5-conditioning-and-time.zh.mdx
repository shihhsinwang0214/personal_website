...
![同一個條件，三個注入的位置](./imgs/l4-5-1.svg)

**圖說：** concat、FiLM 與 cross-attention 適合不同形狀的條件；純量時間步還需要先做頻率嵌入。
...
L4 到這裡，架構與怎麼指揮它都有了，但整個系列一直只在做一件事：**給 $x$ 猜一個 $y$**。接下來的單元 換一個目標——不是猜一個數字，而是把整個分佈寫下來。最大似然、潛在變數與下界、自回歸分解，以及一個比前面任何一篇都難回答的問題：**怎麼判斷一個生成模型好不好。**
</Bridge>
