...
![三種架構，三句不同的「我事先相信」](./imgs/l4-0-1.svg)
...
<Explanation>
第三個選項補一個實務對照：這個實驗在真實影像上做出來的結果是**一樣**的——permuted-MNIST 是一個標準的對照實驗，MLP 在它上面的表現與原始 MNIST 幾乎相同，而 CNN 會大幅下降。CNN 的「下降」才是好消息：它證明那個模型真的在利用空間結構，所以結構被破壞時它會受傷。
</Explanation>
</QuizBlock>
