...
<VizPlan id="ml-l5-0-nll-decomposition" title="一個數字，兩個部分" type="static">
- **檔名**：`l5-probabilistic-modeling/imgs/l5-0-1.svg`（16:9，左右兩格）。
- **左格**：六根長條是真實機率 $(0.05,0.10,0.15,0.20,0.22,0.28)$，上面疊一組 $n=30$ 的經驗長條（半透明、明顯高低不均），再疊一條均勻模型的水平虛線 $1/6$。
- **右格**：三根堆疊長條（$n=30/300/3000$），每一根的下段是經驗熵、上段是 $\mathrm{KL}(\hat p\Vert\text{均勻})$，總高一律 $\log 6=1.7918$（畫一條水平線標出來）。旁邊另畫一根 MLE 的長條，只有下段沒有上段。真實熵 $1.6784$ 畫一條水平參考線穿過三根長條的下段。
- **圖說**：「總高固定是均勻模型的 NLL；下段（熵）隨 $n$ 往真實熵爬，上段（KL）是模型錯的那部分。MLE 把上段壓成零——代價是它連下段的誤差一起照單全收。」
</VizPlan>
...
<Explanation>
$0.7^5=0.16807$，模擬給的 $16.61\%$ 就是它。第三個選項值得多想一步：交叉驗證（<Ref to="ml-cross-validation" mode="title"/>）是用來選超參數的，而這裡 $\hat p$ 是參數；更根本的是，如果五次都是反面，任何只看這份資料的程序都不會告訴你正面存在。這裡真正缺的是資料以外的資訊——也就是先驗。
</Explanation>
</QuizBlock>
...
到這裡，「把分布寫下來」有了一個明確的分數：平均負對數似然，也就是交叉熵，也就是到經驗分布的 KL 加上一個動不了的常數。整個系列前面用過的每一個損失都可以放回這個框架裡讀一次——MSE 是 Gaussian 宣告，交叉熵是類別宣告。

但這個框架有一個前提被悄悄用掉了：**$p_\theta(x)$ 要算得出來**。一旦模型裡有一個沒被觀測到的變數——一個群集標籤、一個潛在編碼、一個加噪聲的中間狀態——$p_\theta(x)$ 就變成一個對那個變數的積分，而那個積分通常沒有封閉解。下一篇處理這件事，答案是一個下界：與其算那個積分，不如找一個猜得夠好的分布，把它變成一個可以最佳化、而且和真值的差距有明確身分的量。
</Bridge>
