...
- **判斷一個病該用哪一個旋鈕。** 對「換一個特徵單位就毀掉訓練」這個老問題，算出動量與 Adam 各能救多少，並說出為什麼標準化仍然不可省。
</Objectives>
...
<VizPlan id="ml-l2-1-three-paths" title="同一個細長碗，三種軌跡" type="static">
- **檔名**：`l2-optimization-in-practice/imgs/l2-1-1.svg`（16:9，左右兩格）。
- **左格**：一個明顯細長的二次碗等高線（$\kappa\approx50$），三條從同一起點出發的軌跡：GD（大幅之字、進展緩慢）、動量 $\beta=0.9$（之字幅度變小、沿溝底明顯加速，末段有輕微過衝）、Adam（幾乎直接沿著對角線走過去，步長均勻）。三條用不同色並標名字與所用的 $\eta$。
- **右格**：縱軸 log 的損失對步數，三條對應曲線，並在右側標出各自兩萬步後的值（$0.0267$／$0.0264$／$0.0164$，直接解 $0.0147$ 畫水平參考線）。另外用淡色再畫一條 $\beta=0.999$ 的動量曲線，標「$\beta$ 才是動量的旋鈕」。
- **圖說**：「動量把之字的幅度壓下來、沿溝底加速；Adam 把每個座標的尺度先除掉，於是走的是一條幾乎直的路。兩者處理的不是同一個病。」
</VizPlan>
...
這條式子解釋了一個很常見的失望：「我加了動量，可是沒什麼差」——因為 $\beta$ 被當成一個不用調的預設值，而它其實是這個方法**唯一**的旋鈕。
...
Adam 的原始論文給的收斂證明後來被指出有誤（Reddi et al., ICLR 2018 的 AMSGrad 一文給了一個 $\beta_2$ 下 Adam 不收斂的反例），修法是對 $s_k$ 取歷史最大值。實務上這個修正很少被採用，因為它在多數任務上沒有可觀察的差別——這是一個「理論反例存在但實務影響小」的例子，值得知道它存在，不必為它改預設值。
...
**動量的旋鈕是 $\beta$，不是「有沒有加」。** $\beta=0.9$ 給 $0.02638$，而 GD 在門檻上是 $0.02666$——差 $1\%$，實務上看不出來。把 $\beta$ 推到 $0.99$ 才掉到 $0.01668$，$0.999$ 是 $0.01596$。而理論算出來的 $\beta^\star=0.998999$ 正好落在後兩者之間，$0.9999$ 又太大（$0.02393$，過衝）。**理論值直接命中實測的最佳位置。**
...
<iframe loading="lazy" src="/personal_website/notes/research_areas/machine-learning-foundations/l2-optimization-in-practice/l2-1-2.html" class="demo-frame" title="三個方法、三個旋鈕，在同一個碗上：互動 demo" style="height: 1132px; width: 100%; border: none;"></iframe>
...
<Option correct>結論下得太早：$\beta$ 是這個方法唯一的旋鈕，而它要跟著條件數長（$\beta^\star\approx1-2/\sqrt\kappa$）。實測 $\beta=0.9$ 幾乎等於 GD（$0.02638$ 對 $0.02666$），$\beta=0.999$ 才掉到 $0.01596$。</Option>
<Option>應該直接換 Adam，動量已經被取代了。</Option>
<Explanation>
$\beta=0.9$ 的記憶長度是十步，所以它最多把有效條件數改善十倍——在 $\kappa=1.6\times10^7$ 面前是零。這題的一般教訓是：**一個方法「沒用」與「它的旋鈕沒調」在實驗上長得一樣**，而區分兩者只需要掃一遍那個旋鈕。第三個選項也不對：本篇實測最好的動量（$0.01596$）比 Adam（$0.01638$）略低。
</Explanation>
</QuizBlock>
...
這一篇把「一個 $\eta$ 背三個工作」拆給兩個方法：動量記住方向、買到有效條件數（$\kappa\to\sqrt\kappa$，但 $\beta$ 必須跟著 $\kappa$ 長——$0.9$ 幾乎等於沒加），Adam 記住每個座標的尺度、買到對梯度尺度的免疫（$\eta$ 掃 30 倍同一個結果）。而那個「換一個特徵單位」的老問題，兩者都救不掉，因為它們改的是走路的方式，不是碗的形狀。

兩個方法都靠**同一個機制**：把過去的量做移動平均。下一篇把這個機制推到一個更棘手的情況——**目標本身也在動**。當訓練的標籤是模型自己產生的（自我一致性、自我蒸餾、任何 bootstrapping），移動平均與 stop-gradient 就從「加速技巧」變成「會不會塌掉」的關鍵，而它們各自負責的事情常常被搞混。
</Bridge>
