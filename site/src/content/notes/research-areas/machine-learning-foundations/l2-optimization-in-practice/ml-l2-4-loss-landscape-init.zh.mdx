...
<Bridge to="ml-mlp">
一路上我們一直在用「一個模型」而沒有問它長什麼樣子——多項式基底、或一個兩層的 $\tanh$ 網路，都只是被當成「有參數可以調的東西」。接下來的單元 換這個問題：**架構本身在宣告什麼**。從最沒有結構的 MLP 開始——它為什麼「什麼都能擬合」，以及為什麼那句話幾乎不含任何實用資訊。
</Bridge>
