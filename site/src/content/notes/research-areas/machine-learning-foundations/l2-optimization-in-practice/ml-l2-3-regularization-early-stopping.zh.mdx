...
![限制容量與限制時間，走的是同一條路](./imgs/l2-3-1.svg)
...
這條對應有三個立刻可用的推論。**一**，提早停止的「正則化強度」和學習率綁在一起：同樣停在第 1000 步，$\eta$ 大兩倍等於 $\lambda$ 小兩倍。**二**，先大後小的學習率 schedule 同時在改變這個隱含的 $\lambda$，所以「調 schedule」與「調 weight decay」不是兩個獨立的設定。**三**，如果你已經用了明確的 weight decay，提早停止能再多買到的東西就有限了。
...
<Explanation>
這個等價的實務價值在於：它告訴你這兩個設定**不要同時亂調**。如果已經有明確的 weight decay，再加提早停止能多買到的有限；反過來，改學習率 schedule 就已經在改動那個隱含的 $\lambda$。第三個選項不算錯（省時間是真的），但它沒有回答「為什麼分數一樣」。
</Explanation>
</QuizBlock>
...
L2 到目前為止的每一個結論，都建立在一個沒有講出來的假設上：**損失曲面是一個碗，最小點只有一個。** 二次問題確實如此，但真實網路不是。接著處理這件事：同一張地圖從不同地方出發會不會到同一個谷底、初始化的尺度為什麼是一個要調的設定，以及「深就會梯度消失」這個說法在實測上站不站得住。
</Bridge>
