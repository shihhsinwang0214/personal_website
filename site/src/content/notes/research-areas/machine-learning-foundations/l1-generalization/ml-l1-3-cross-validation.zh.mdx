...
現在你有了可信的數字。最後一個問題是**怎麼讀它們**：模型不夠好時，該去收更多資料、換更大的模型、還是多訓幾輪？這三件事都要花錢或花時間，而且處方彼此不能替代。接著把訓練與驗證誤差對「資料量、容量、訓練步數」畫成三種曲線，每一種形狀對應一個明確的處方——這是 L1 這一整個 class 的收束。
</Bridge>
