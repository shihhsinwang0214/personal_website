...
- 說出「模型太複雜」不是一個絕對判斷，並用同一份 toy 在兩種資料量下把這句話證明出來。
- 判斷一個停在正值不動的訓練誤差是不是問題——並說出要拿什麼數字來比。
</Objectives>
...
中間某處有一個最佳點。**但這個點不是模型的性質，是「模型與資料量」這一對的性質**——旋鈕的最佳位置隨 $n$ 移動。這是本篇最重要的一句話，因為它決定了你聽到「這個模型太複雜」時該問的下一個問題：**相對於多少資料？**

<VizPlan id="ml-l1-0-four-cells" title="四格診斷，與同一個旋鈕的兩端" type="static">
- **檔名**：`l1-generalization/imgs/l1-0-1.svg`（16:9，左右兩格）。
- **左格**：$2\times2$ 的診斷方格，橫軸「泛化誤差」、縱軸「訓練誤差」。四格分別標「成功」「過擬合」「欠擬合」「幾乎不可能——先查 bug」，每格附一行處方。甲、乙兩個學生標成兩個點放進對應的格子。
- **右格**：共用 toy 的三張小圖並排，同一組 30 個點：degree 1（直線，明顯抓不到彎）、degree 5（貼合真值）、degree 25（穿過每一點、在點與點之間狂震），真值 $\sin$ 用灰虛線。每張下方標訓練誤差與母體風險（$0.2118/0.2679$、$0.0147/0.0280$、$0.0016/8032$）。
- **圖說**：「同一個旋鈕的兩端是兩種相反的病。中間那一格的位置不是模型的性質，是模型與資料量這一對的性質。」
</VizPlan>
...
**同一個旋鈕，換一個資料量。** 把 $n$ 從 30 加到 300，其他完全不動：
...
**U 形不見了。** $d=25$ 在 $n=30$ 時是 $8032$，在 $n=300$ 時是 $0.0283$——同一個模型、同一個旋鈕位置，從災難變成幾乎最佳。而且從 $d=9$ 到 $d=20$，母體風險穩定停在 $0.024$ 附近，緊貼不可約誤差 $0.0225$：**多出來的容量沒有造成傷害。**
...
<iframe loading="lazy" src="/personal_website/notes/research_areas/machine-learning-foundations/l1-generalization/l1-0-2.html" class="demo-frame" title="轉旋鈕，看 U 形怎麼隨資料量移動：互動 demo" style="height: 823px; width: 100%; border: none;"></iframe>
...
<Explanation>
這題在練習把「模型複雜度」這個常見的名詞從絕對量改成相對量。實務上的形式是：同一個架構在 1 萬筆與 1 億筆資料上是兩個完全不同的問題，該調的旋鈕、該擔心的病都不一樣。順帶一提，$n=300$ 那張表裡 $d=9$ 到 $20$ 全部停在 $0.024$，示範了「多餘容量在資料充足時可以是無害的」——現代深度學習大量依賴這個區域。
</Explanation>
</QuizBlock>
...
這一篇把診斷從「一個數字」升級成「兩個數字的組合」：四格裡三格會發生，各有相反的處方，而第四格是 bug 的訊號。U 形的谷底不是模型的性質而是模型與資料量這一對的性質——$n$ 從 30 加到 300，$d=25$ 就從 $8032$ 變成 $0.0283$。

但「過擬合」目前還是一個定性的說法。下一篇把母體風險真正拆開成三塊可以分別測量的量：**bias²、variance、不可約誤差**。拆完之後，U 形的兩側各自對應哪一塊、加資料為什麼只壓得到其中一塊、以及為什麼「十個房仲各看不同的一百間房」是理解 variance 最好的圖像，都會變得具體。
</Bridge>
