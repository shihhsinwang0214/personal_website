...
<Objectives>
- 把「每個位置一支箭頭」寫成 vector field $u(x,t)$，把「順著箭頭走」寫成 ODE $\dot x=u(x,t)$，並說出解「存在且唯一」依賴的條件。
- 由唯一性推出「兩條軌跡不會在同一時刻交叉」，並說出這件事在時間會變的 vector field 上要怎麼修正。
- 對「葉子放在河面上會漂到哪」這類情境，判斷「順著箭頭一步步走」這個做法何時可靠、依賴什麼假設，並用一個小模擬驗證。
</Objectives>
...
<VizPlan id="m2-0-field" title="水流地圖與一片葉子的軌跡" type="static">
- **檔名**：`m2-deterministic-dynamics/imgs/m2-0-1.svg`，16:9。
- **版面**：一條由左往右的河，河面畫 $12\times6$ 網格的短箭頭（vector field $u(x,y)=(1,\;0.5\cos x)$：往右流，同時上下微微擺動），靠岸箭頭短、中間箭頭長。
- 一片葉子從左側 $(0,0)$ 出發，畫出它的軌跡 $y=0.5\sin x$，軌跡上每隔一段放一個小圓點，並在三個點旁畫出「那一點的箭頭剛好是軌跡的切線」。
- 另一片葉子從 $(0,0.6)$ 出發，畫出平行擺動的第二條軌跡；兩條線**靠近但不相碰**，旁邊小字「同一時刻不會交叉」。
- 右下角小格：把同一點放兩片葉子，畫成一條線上的兩個重疊的圓點，小字「同一起點 → 同一條路」。
- **圖說**：「水流地圖是 vector field $u(x,t)$：每點一支箭頭。葉子的軌跡 $x(t)$ 處處貼著箭頭走，寫成 ODE 就是 $\dot x=u(x,t)$。」
</VizPlan>
...
<VizPlan id="m2-0-two-leaves" title="兩片葉子、同一個起點：Lipschitz 與非 Lipschitz 的河" type="interactive">
- **位置**：本節之後。**檔名**：`m2-deterministic-dynamics/m2-0-1.html`。
- **版面**：左右兩格，各是一條河的俯視圖，畫滿短箭頭。左格是上面的 smooth 場 $u=(1,0.5\cos x)$；右格是一條在中線附近**非 Lipschitz** 的河：$u=(1,\;\mathrm{sign}(y)\,|y|^{1/3})$——離中線越近，箭頭轉得越急。
- **控制項**：滑桿「步長 $h$」（$1,\,0.3,\,0.1,\,0.03$）；滑桿「兩片葉子的起點間距 $\delta$」（$0$ 到 $0.1$，含 $0$）；按鈕「放葉子」逐步畫出兩條軌跡（不自動循環，可按「再放一次」）。
- **行為**：左格不論 $h$、$\delta$ 怎麼調，兩條軌跡幾乎重合、與淡灰色的 closed form 解貼著走，且縮小 $h$ 差距等比例縮小。右格當起點在中線 $y=0$ 附近、$\delta$ 極小或為 $0$ 時，兩片葉子會被數值誤差推到中線兩側後**分道揚鑣**；hover 任一點顯示該處的 Jacobian 大小，中線附近顯示「$\partial_y u\to\infty$，Lipschitz 常數不存在」。
- **開關**「時間會變的場」：把左格改成 $u=(1,0.5\cos(x-t))$，並額外畫出 $(x,t)$ 的側視小圖，讓讀者看到兩條軌跡在空間中可以穿過同一點（不同時刻），但在 $(x,t)$ 圖裡不交叉。
- **底部觀察句**：「觀察：箭頭變化不劇烈（Lipschitz）的河面上，同一點出發只有一條路，兩條路不會在同一刻相交；在箭頭變化無限急的中線上，同一點出發的兩片葉子可以漂去不同地方——決定它們命運的不再是地圖，而是雜訊。」
- **匯出 PNG**：兩格併成 16:9，含當前 $h$、$\delta$ 與軌跡。
- **註記**：右格的分道揚鑣不是程式錯誤，是 ODE 本身沒有唯一解；再小的 $h$ 都會發生，只是分開的位置變得更難預測。
</VizPlan>
...
本篇把「每點一支箭頭」寫成 vector field、「順著箭頭走」寫成 ODE，並用 Lipschitz 條件換到解的存在唯一與軌跡不交叉。一片葉子的問題解決了；下一篇同時放下一千片葉子，問的不再是「一片葉子在哪」，而是「這群葉子的分佈變成什麼樣」。
</Bridge>
