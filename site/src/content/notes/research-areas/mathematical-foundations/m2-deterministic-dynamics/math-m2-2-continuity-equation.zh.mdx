...
- **說出這條方程式能預測什麼（給 $p_0$ 與 $u$，直接得到所有 $p_t$）、依賴什麼假設（質量守恆、$u$ 夠 smooth）。** 它和 pushforward 是同一件事的兩種寫法。
...
![單一地磚的四邊 flux 與整片格網的 continuity equation 記帳。](./imgs/m2-2-1.svg)
...
本篇用「每塊地磚進 − 出」推出 continuity equation，並看到它與 pushforward 是同一個 $p_t$ 的兩種寫法、以及同一條分佈路徑有無限多個 $u$。到這裡我們知道葉子與分佈「理論上」往哪走；接著回到電腦上那個「每隔一小段看一次箭頭」的做法，問它到底差多遠。
</Bridge>
