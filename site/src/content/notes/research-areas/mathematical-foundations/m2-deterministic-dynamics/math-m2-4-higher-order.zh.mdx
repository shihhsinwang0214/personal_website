...
<Objectives>
- 寫出 Heun 法與 midpoint 法的更新式，並用 Taylor 展開說明它們的局部誤差為什麼是 $h^3$（全域二階）。
- 說出「階數」與「每步成本」的取捨：在**相同的方向讀取次數**下比較誤差，並說出高階方法何時贏、何時不贏。
- 對「導航多讀一次方向值不值得」這類情境做判斷、說出它依賴的假設（路夠 smooth、預算夠多），並用一個小模擬在相同讀取次數下對照兩種方法。
</Objectives>
...
<VizPlan id="m2-4-chord" title="切線走、弦走：Euler 與 midpoint 在一步裡的差別" type="static">
- **檔名**：`m2-deterministic-dynamics/imgs/m2-4-1.svg`，16:9。
- **版面**：左格一段圓弧，起點 $A$、真實終點 $B$、中點 $M$。從 $A$ 畫切線（Euler 一步，終點 $E$ 落在圓外），標「$\|EB\|\sim h^2$」；再從 $A$ 畫一條與 $M$ 處切線平行的線段（midpoint 一步，終點 $P$ 幾乎落在 $B$ 上），標「$\|PB\|\sim h^3$」。$M$ 處的切線用虛線畫出，並標「與弦 $AB$ 平行」。
- 中格：Heun 的畫法——從 $A$ 畫切線到 $E$，在 $E$ 讀方向（畫一支箭頭），把 $A$ 與 $E$ 兩支箭頭平均後從 $A$ 走一步，終點也幾乎落在 $B$。
- 右格：三條完整的四分之一圓 Euler／Heun／midpoint 折線疊在圓弧上（$h=30$ s，各六步），終點分別標 $486$、$23$、$7$ m。
- **圖說**：「Euler 用起點方向直走，離開彎路的量是 $h^2$；midpoint 用中點方向、Heun 用兩端平均，都消掉了 $h^2$ 那一項，一步只剩 $h^3$ 的誤差。」
</VizPlan>
...
<VizPlan id="m2-4-nfe" title="同樣的讀取次數：一階 vs 二階，平滑路 vs 尖角路" type="interactive">
- **位置**：本節之後。**檔名**：`m2-deterministic-dynamics/m2-4-1.html`。
- **版面**：左格俯視道路，疊上三條折線（Euler、Heun、midpoint）與真實路徑；右格是 log–log 圖，橫軸是總讀取次數 NFE、縱軸是終點誤差，三條方法各一條曲線，並畫斜率 $-1$、$-2$ 的參考虛線。
- **控制項**：滑桿「總讀取次數 NFE」（$4$–$400$，對數）；下拉「路型」：等速圓弧、S 形彎、**帶一個 90 度尖角的折線路**、直路加減速；開關「顯示每步的局部誤差」在折線頂點上畫小圓、半徑正比於該步局部誤差。
- **行為**：圓弧路上拖 NFE 往右，右格三條線分開——Euler 貼著斜率 $-1$、另兩條貼著斜率 $-2$，midpoint 常數約是 Heun 的一半。拖 NFE 到最左（$4$–$6$ 次）時三條線接近甚至交錯，畫面顯示「預算極少時階數優勢不保證」。
- **切到尖角路**：右格三條線在中等 NFE 以後全部變成斜率 $-1$，並在圖上標出「尖角處 Taylor 展開失效，所有方法退回一階」；左格在尖角那一步的局部誤差小圓變得特別大，三種方法差不多大。
- **Hover** 右格任一點：顯示該 NFE 下三種方法的步長 $h$ 與誤差數字，並列出「Euler 需要多少 NFE 才能達到 midpoint 在此的誤差」。
- **底部觀察句**：「觀察：多讀一次方向不是把誤差減半，而是消掉 $h^2$ 那一項，換成 $h^3$——同樣的讀取次數下差一到兩個數量級。但這件事靠的是路夠 smooth；遇到尖角，所有方法一起退回一階。」
- **匯出 PNG**：兩格併成 16:9，含當前 NFE 與路型。
</VizPlan>
...
本篇用「在中點多看一眼」把誤差從一階推到二階，並學會用相同的讀取次數做公平比較、以及在路不 smooth 時保持懷疑。到此為止，河面上的水流是確定的、葉子的路是確定的。下一個 class 把亂流放進河裡：先從最簡單的隨機運動——每秒隨機往左或往右一步——開始，問一小時後離起點多遠。
</Bridge>
