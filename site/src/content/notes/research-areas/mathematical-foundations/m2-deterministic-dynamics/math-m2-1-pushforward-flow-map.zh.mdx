...
<Objectives>
- 把「每個起點在時刻 $t$ 被送到哪」打包成 flow map $\Phi_t$，並說出它為什麼是可逆的。
- 把「一群葉子的分佈被水流搬走」寫成 pushforward $p_t=(\Phi_t)_\#p_0$，並用 change of variables 寫出密度的公式。
- 對「一群葉子一分鐘後的分佈」這類情境，判斷「數一千片葉子畫直方圖」與「用公式算密度」各自依賴什麼假設、誤差在哪，並用一個小模擬對照兩者。
</Objectives>
...
<VizPlan id="m2-1-jelly" title="一團葉子被水流搬走並變形" type="static">
- **檔名**：`m2-deterministic-dynamics/imgs/m2-1-1.svg`，16:9。
- **版面**：左中右三格，同一條河（俯視），vector field 用淡灰短箭頭鋪底。場選一個會「先加速再匯合」的：$u(x,y)=(1+0.5x/\ell,\;-0.5\,y)$（$\ell$ 是一個空間尺度，和 Lipschitz 常數 $L$ 無關）（往右越流越快，同時往中線收攏）。
- 左格 $t=0$：橋下一個圓形的點雲（$\mathcal N(0,\sigma^2 I)$ 抽 300 點），畫出五個等密度的同心圓（等高線），並在點雲上疊一個 $3\times3$ 的方格網。
- 中格 $t=20$、右格 $t=40$：同一批點被 $\Phi_t$ 送到的位置；方格網跟著變形——橫向被拉長（加速）、縱向被壓扁（匯合）；等高線變成拉長的橢圓。每個格子裡的**點數不變**（在角落標出其中一格的點數，三格都是同一個數字），但面積變了。
- 三格之間用箭頭連接，箭頭上寫 $\Phi_{20}$、$\Phi_{40}$。
- **圖說**：「水流不會讓兩片葉子重合，也不會把葉子群撕開，所以每一小塊裡的葉子數守恆；密度的變化完全來自那一小塊面積的伸縮。這就是 pushforward 的畫面。」
</VizPlan>
...
<VizPlan id="m2-1-pushforward" title="撒一千片葉子 vs. 用 change of variables 算密度" type="interactive">
- **位置**：本節之後。**檔名**：`m2-deterministic-dynamics/m2-1-1.html`。
- **版面**：上半是俯視的河（2D），下半是沿中線的 1D 密度圖。上半畫 vector field 的短箭頭；點雲從 $p_0=\mathcal N(0,\sigma^2I)$ 抽出，隨時間滑桿 $t\in[0,40]$ 被 $\Phi_t$ 送走（每片葉子用 ODE 數值積分）。
- **控制項**：滑桿 $t$；滑桿「葉子數 $N$」（$100,\,1000,\,10000$，對數）；下拉選單「水流地圖」：(a) 保體積的旋轉場 $u=(-y,x)$，(b) 收攏場 $u=(1,-0.5y)$，(c) 加速場 $u=(1+0.5x/\ell,\,0)$，(d) **非 Lipschitz** 的收攏場 $u=(1,-\mathrm{sign}(y)\sqrt{|y|})$。
- **下半**：兩條曲線疊在一起——葉子位置在 $y$ 方向的直方圖（柱狀）、以及公式 $p_0(\Phi_t^{-1}(y))\,|\det\nabla\Phi_t^{-1}(y)|$ 算出的密度（實線，用反向 ODE 加數值 Jacobian）。右上角顯示兩者的最大差距。
- **行為**：場 (a) 等高線只旋轉不變形，直方圖與公式重合；(b)、(c) 等高線被壓扁或拉長，公式曲線變高變窄或變低變寬，直方圖跟著走；$N$ 越大兩者越貼合，$N=100$ 時直方圖明顯毛躁但公式曲線不變。場 (d) 拉到 $t$ 夠大時，所有葉子在有限時間內被收到中線 $y=0$（$\sqrt{|y|}$ 場會在有限時間到達零），直方圖在 $y=0$ 堆成一根極高的柱子，公式曲線在中線發散、顯示「Jacobian → 0，密度公式失效」的警示字。
- **Hover** 上半任一片葉子：畫出它的整條軌跡與起點，並在下半標出它的起點密度 $p_0(x_0)$ 與終點密度 $p_t(y)$、以及比值 $=1/|\det\nabla\Phi_t|$。
- **底部觀察句**：「觀察：數葉子與算公式給的是同一個 $p_t$；前者的誤差是有限樣本，後者沒有取樣誤差但要可逆。保體積的場只搬形狀不改密度；一般的場按 Jacobian 反比改密度；非 Lipschitz 的場能把葉子擠成一點，公式在那裡失效。」
- **匯出 PNG**：上下兩半併成 16:9，含當前 $t$、$N$ 與場的名字。
</VizPlan>
...
本篇把所有起點的終點打包成 flow map $\Phi_t$，把分佈的搬運寫成 pushforward，並由可逆性得到密度的 change of variables 公式。但這條公式要先算出整個 $\Phi_t$；下一篇換一條路：不追葉子，直接寫下密度 $p_t$ 自己滿足的方程式。
</Bridge>
