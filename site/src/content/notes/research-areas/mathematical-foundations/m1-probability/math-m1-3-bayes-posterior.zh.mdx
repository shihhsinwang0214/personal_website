...
<Objectives>
- 寫出 Bayes 定理的三個成分——prior、likelihood、posterior——並用「一萬個人」的數法把快篩陽性後的機率真的算出來。
- 說出 posterior 為什麼是「加權平均的權重」：對所有可能的真相取 conditional expectation 時，用的就是它；並在 prior 與 likelihood 都是 Gaussian 的情形下配方算出 posterior，看到那組權重就是兩邊的精確度。
- 對「準確率 95% 所以陽性就是 95% 有病」這個直覺，說出它漏掉哪個假設、在什麼情況下反而接近正確；並用模擬驗證，包括重複檢測的錯誤不獨立時結論如何改變。
</Objectives>
...
<VizPlan id="m1-3-icon-array" title="一萬個人的篩檢：陽性的人裡有多少真的有病" type="static">
- **檔名**：`m1-probability/imgs/m1-3-1.svg`（16:9）。
- **左半**：$100\times100$ 的小方格陣列代表一萬人。有病的 100 格（左上角一小塊）著深色，沒病的 9,900 格淺色。在每一塊裡把「驗出陽性」的格子打上一個 $+$ 記號：深色塊裡 95 格有 $+$，淺色塊裡 495 格有 $+$（隨機散佈）。
- **右半**：只把有 $+$ 的格子抽出來排成一排（590 格），深色 95 格集中在左端、淺色 495 格在右。上方一個大括號寫「陽性的人」，深色段上方標 $95/590\approx16\%$。
- **底部一行**：「sensitivity 與 specificity 說的是**每一塊裡**打 $+$ 的比例；你想知道的是**打了 $+$ 的那一排**裡深色的比例。分母不同。」
- **圖說**：「95% 的準確率沒有錯；錯的是忘了淺色那一塊有 9,900 人，5% 就是 495 個假陽性。」
</VizPlan>
...
值得把方法本身記起來：**這裡乘除的是機率密度，不是隨機變數**（兩個 Gaussian 隨機變數相乘不會是 Gaussian）。而 Gaussian 的密度是「$\exp$（一個 $x$ 的二次式）」，相乘相除只是把指數上的二次式加加減減，加減完還是二次式——所以「配出來還是 Gaussian」這件事不必逐次驗算，看指數是不是 $x$ 的二次式就知道。
</Try>
...
<VizPlan id="m1-3-bayes" title="拖盛行率、拖準確率、再測一次：posterior 怎麼動" type="interactive">
- **位置**：本節之後。**檔名**：`site/public/notes/research_areas/mathematical-foundations/m1-probability/m1-3-2.html`。這是本篇的核心互動。
- **版面**：左格是 $100\times100$ 的人口方格（對應靜態圖），依當前參數即時重畫有病／沒病與 $+$ 記號；右格上方是「陽性的人那一排」的長條（深淺兩段，標出 posterior 的百分比），右格下方是一條曲線：posterior 對盛行率（橫軸對數，$0.1\%$–$50\%$），一個游標標出目前的盛行率。
- **控制項**：滑桿「盛行率」（$0.1\%$–$50\%$）、「sensitivity」、「specificity」（各 $50\%$–$99.9\%$）；開關「用 odds 顯示」：右上改成 prior odds × likelihood ratio ＝ posterior odds 的算式；按鈕「再測一次（獨立）」：對陽性的人再做一次，長條更新為兩次皆陽性者的組成；按鈕「再測一次（相關：假陽性者必再陽性）」：長條幾乎不變並標紅「沒有新資訊」；按鈕「重置」。
- **Hover** 右下曲線任一點顯示該盛行率下的 posterior，並在曲線上標出「posterior $=$ sensitivity」發生的盛行率（約 50%）——直覺答案正確的地方。
- **底部觀察句**：「觀察：posterior $\propto$ likelihood × prior。同一支快篩，prior 從 1% 拉到 30%，陽性後有病的機率從 16% 變成 89%。重複檢測只有在錯誤獨立時才能一直乘。」
...
本篇把「陽性後有病的機率」算成 16%，看到直覺的 95% 是忘了 prior，並確認 posterior 是「對看不見的真相取平均」時的權重。現在手上有三件東西：最佳猜測是 conditional expectation、conditional expectation 是用 posterior 加權的平均、而微分可以穿過積分。下一篇把它們接在一起，處理一個看似平凡的問題——體重計讀 72.3 kg、雜訊 ±1 kg，最佳猜測真的是 72.3 嗎？
</Bridge>
