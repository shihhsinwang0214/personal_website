...
![同一個兩點分布的 mean、mode、median，以及平方與絕對誤差曲線。](./imgs/m1-2-1.svg)
...
本篇用三行證明把「平方誤差最小的策略」釘在 $\mathbb E[Y\mid X]$ 上，並看到換計分就換答案：絕對誤差選中位數、不對稱懲罰選 quantile。「最可能的」是另一種計分的答案，不是平方誤差的。到這裡，「猜」的目標已經清楚——桶內平均。接著處理「桶」本身怎麼算：看到一個觀測結果之後，各種可能的真相各佔多少比例？快篩陽性是最好的例子。
</Bridge>
