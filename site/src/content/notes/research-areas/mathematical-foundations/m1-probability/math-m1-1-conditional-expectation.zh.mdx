...
<Objectives>
- 說出 $\mathbb E[Y\mid X=x]$ 是「把 $X=x$ 的人分成一桶再取平均」，並區分 $\mathbb E[Y\mid X=x]$（一個數）與 $\mathbb E[Y\mid X]$（$X$ 的函數、一個隨機變數）。
- 寫出 tower property $\mathbb E\big[\mathbb E[Y\mid X,G]\,\big|\,X\big]=\mathbb E[Y\mid X]$，用「細桶合併回粗桶」解釋它，並給出四行證明。
- 對「多知道一個條件就會猜得更好」這個說法，判斷它在什麼意義下對、從資料估計時的代價是什麼，並用模擬驗證桶太細時會發生的事。
</Objectives>
...
<VizPlan id="m1-1-buckets" title="按鞋碼分桶，再按性別細分：小桶合併回大桶" type="static">
- **檔名**：`m1-probability/imgs/m1-1-1.svg`（16:9）。
- **上排**：六個桶，標鞋碼 23–28，每桶上方寫平均身高（例如 158、161、165.2、169、173、177——鞋碼 25 那一桶要和下排合併回來的數字**完全一致**），桶身高度＝人數。桶內用小點畫出身高的散佈（縱向），讓讀者看到每桶內仍有一整個範圍。
- **下排**：把鞋碼 25 那一桶拆成兩個小桶——「男・25」（人數 40%，平均 170）與「女・25」（人數 60%，平均 162），各自的散佈比大桶窄。兩小桶之間一個 $\Rightarrow$ 指回大桶，旁邊寫 $0.4\times170+0.6\times162=165.2$。
- **右側一句**：「猜測是一張表（每個鞋碼一格），不是一個數。細分再合併，平均不變；桶內的散佈變小。」
- **圖說**：「$\mathbb E[Y\mid X]$ 是桶上的數字；tower property 是『小桶按人數加權平均＝大桶』。」
</VizPlan>
...
<VizPlan id="m1-1-scatter" title="鞋碼 vs 身高：拖桶寬、開性別、縮樣本" type="interactive">
- **位置**：本節之後。**檔名**：`site/public/notes/research_areas/mathematical-foundations/m1-probability/m1-1-2.html`。這是本篇的核心互動。
- **版面**：主圖是鞋碼（橫軸）對身高（縱軸）的散佈圖，點依性別著色（預設不上色，開關後上色）。疊上一條階梯狀的曲線：每個桶的平均身高 $\mathbb E[Y\mid X\in\text{桶}]$。右側面板顯示滑鼠所在桶的：人數、平均、男女比例、男女各自的平均、以及「加權回來」的數字。
- **控制項**：滑桿「桶寬」（$0.1$–$2$ 公分）；開關「依性別細分」：階梯曲線分裂成兩條（男、女），面板同時顯示 $0.4\times170+0.6\times162$ 這種算式並打勾「＝大桶平均」；滑桿「樣本數」$n$（$50$–$200{,}000$，對數）；按鈕「重新抽樣」；開關「顯示每桶的標準差」：在階梯上畫出 ±1 標準差的帶子，細分後帶子變窄。
- **預期看到的事**：$n$ 大時階梯平滑、細分後兩條曲線各自平滑且加權回來完全吻合；$n$ 小或桶太窄時，階梯鋸齒、部分桶為空（畫成缺口）、細分後更糟——用紅字標「桶內 $<5$ 人」。
- **底部觀察句**：「觀察：條件多一個，平均不會系統性移動（tower property），但每桶內的散佈變小；代價是每桶的樣本變少，估出來的表變抖。」
- **匯出 PNG**：主圖 + 面板併成 16:9。
</VizPlan>
...
本篇把「用已知條件猜」寫成 $\mathbb E[Y\mid X]$——分桶取平均的那張表——並用 tower property 說明多一個條件不會移動平均，只會縮小每桶的散佈；從資料估表時，桶太細是代價。還沒回答的是：為什麼「桶的平均」是好猜測？下一篇換一個情境問同一件事——天氣預報每天報一個數字，什麼策略讓長期的誤差最小？
</Bridge>
