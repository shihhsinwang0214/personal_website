...
<Objectives>
- 寫出 entropic OT $\min_\pi\langle\pi,c\rangle-\varepsilon H(\pi)$，說出它的最佳解有 $\pi=\mathrm{diag}(u)\,K\,\mathrm{diag}(v)$、$K=e^{-c/\varepsilon}$ 的形狀，以及 Sinkhorn 演算法為什麼只是「交替把列和欄正規化」。
- 說出 $\varepsilon$ 的作用：$\varepsilon\to0$ 回到精確 OT、$\varepsilon\to\infty$ 變成獨立 coupling；entropic 解比精確解「拆得更散」、成本偏高。
- 說出 minibatch 近似的偏差方向：在小組內配對得到的距離期望**高於**真實 $W_2$、得到的配對比真實最佳配對「更亂」，且偏差隨 batch 變大而縮小；並對一個大規模情境判斷該用哪種近似、驗證偏差的方向與大小。
</Objectives>
...
<VizPlan id="m6-3-two-ways" title="磨圓稜角 vs 只看一小塊" type="static">
- **檔名**：`m6-optimal-transport/imgs/m6-3-1.svg`，16:9，三欄。
- **左欄「精確」**：一張 $8\times8$ 的運輸表熱圖，只有沿一條「對角帶」的 15 格有值（頂點），其餘全白；標「最多 $m+n-1$ 格」。
- **中欄「entropic」**：同一對邊際，三張熱圖並排 $\varepsilon=0.01,\,0.1,\,1$：從幾乎等於左欄、到對角帶變寬變模糊、到幾乎均勻一片；下方標「$\pi=\mathrm{diag}(u)\,K\,\mathrm{diag}(v)$」與各自的成本（單調上升）。
- **右欄「minibatch」**：整個大點雲淡色為背景，隨機圈出兩小組（各 8 點）並在組內畫最佳配對線；其中幾條線明顯很長、跨過了背景裡「本來有更近的店」的位置，用紅色標出；下方小字「組內被迫配 → 距離偏高、配對偏亂」。
- **圖說**：「精確解是多面體的頂點、有稜角、難找。entropic OT 用 $\varepsilon$ 把它磨圓，解變成兩個對角縮放夾一個矩陣。minibatch 只在小組裡精確解，但小組代表不了全體，配出來的距離偏高。」
</VizPlan>
...
這裡有一個實作上很會騙人的地方，值得自己踩一次。把 `iters` 從 1000 降回 300，$\varepsilon=0.05$ 會給出 $10.49$——**比精確解 $10.73$ 還低**，直接違反上面那個「entropic 解的成本不低於真實 OT」。原因不是定理錯了，是那個 $P$ 還不是合法的 coupling：300 輪之後列邊際的 $L_1$ 誤差還有 $0.038$，質量沒有守好，成本自然可以被壓到下界以下。$\varepsilon$ 越小收斂越慢，所以**看到「比精確解還低」的第一件事是去檢查邊際**，不是去懷疑定理。
...
<VizPlan id="m6-3-approx" title="拖動 ε 與 B：兩種近似怎麼偏、偏多少" type="interactive">
- **位置**：本節之後。**檔名**：`m6-optimal-transport/m6-3-1.html`。
- **版面**：左格是兩團二維點雲（各 300 點）與當前 coupling 的連線（$\pi_{ij}$ 大於門檻才畫、粗細 ∝ $\pi_{ij}$）；右格上方是「成本 vs $\varepsilon$」與「成本 vs $B$」兩條曲線疊在同一張圖（橫軸分別是 $\log\varepsilon$ 與 $\log B$，各自標軸），一條水平線標精確 OT 的成本（用 LP 算一次）；右格下方顯示當前 coupling 的 entropy 與「每個 $x$ 平均被配到幾個 $y$」。
- **控制項**：模式切換「Sinkhorn／minibatch」；Sinkhorn 模式有 $\varepsilon$ 滑桿（$10^{-2}$–$10^1$，對數）與「單步」按鈕（一次只做一輪列／欄正規化，畫面高亮當前正在修的是列和還是欄和，並顯示兩組約束各自的違反量——讀者看到輪流修、逐步收斂）；minibatch 模式有 $B$ 滑桿（$2$–$300$）與「再抽一組」按鈕（左格圈出這一組並畫組內最佳配對，累計平均成本更新右圖）。
- **行為**：$\varepsilon$ 拉大：連線從幾乎一對一變成每個點連出一把扇形，成本曲線上升、entropy 上升；$\varepsilon$ 拉到最小：畫面標「$K$ 下溢，切換 log 域」並自動切換。$B$ 拉小：每次抽組畫出的配對線有明顯的長線，累計成本高於水平線；$B$ 拉大：逼近水平線。
- **開關「$\mu=\nu$」**：把第二團換成第一團的另一組樣本，精確 OT 成本的水平線落到 0 附近，minibatch 曲線仍明顯為正——標「相同分佈被判成有差」。
- **開關「Sinkhorn divergence」**：右圖多一條扣掉自我運輸成本的曲線，對 $\varepsilon$ 平坦得多。
- **底部觀察句**：「觀察：Sinkhorn 把稜角磨圓，解是 $\mathrm{diag}(u)K\mathrm{diag}(v)$、輪流正規化就收斂，成本隨 $\varepsilon$ 偏高、配對偏散；minibatch 只在小組裡精確解，成本隨 $1/B$ 偏高、配對偏亂。兩個旋鈕都在『精確 OT』與『隨便配』之間插值。」
- **匯出 PNG**：左右併成 16:9，含當前模式、$\varepsilon$ 或 $B$、成本與 entropy。
</VizPlan>
...
<Bridge>
本篇看了大規模 OT 的兩條近似路：Sinkhorn 用 $\varepsilon$ 把解磨圓成 $\mathrm{diag}(u)K\mathrm{diag}(v)$、輪流正規化就收斂；minibatch 用 $B$ 只在小組裡精確解。兩者都給出比精確解更分散、成本偏高的 coupling，而且都在「精確 OT」與「隨機配」之間插值。到這裡，Mathematical Foundations 的七個 class——calculus 工具箱、機率與 conditional expectation、確定與隨機的動力學、Markov chain、optimization 與距離、optimal transport——全部建好了。每一篇的末尾都列出了它在哪些課程裡被重用；當你在某門課裡遇到一個工具覺得陌生，回到這裡對應的那一篇，讀十分鐘，再回去。
</Bridge>
