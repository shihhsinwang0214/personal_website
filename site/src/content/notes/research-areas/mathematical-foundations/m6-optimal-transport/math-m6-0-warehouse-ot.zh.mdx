...
<Objectives>
- 把「搬貨最省油」寫成兩個數學問題：Monge（每個來源只送一個目的地，找一個映射）與 Kantorovich（允許拆貨，找一張運輸表），並說出後者的三個約束。
- 說出 coupling（運輸表）是什麼——一個 joint distribution，兩個 marginal 分別是「供給」與「需求」——以及為什麼 Kantorovich 的問題永遠有解、Monge 的不一定。
- 對一個「配對／分配」的生活情境判斷該用哪種問法、說出答案依賴的假設（成本怎麼定、貨能不能拆），並用一個小例子算出最佳解與它省了多少。
</Objectives>
...
<VizPlan id="m6-0-table" title="運輸表：每一列、每一欄都要對得上" type="static">
- **檔名**：`m6-optimal-transport/imgs/m6-0-1.svg`，16:9。
- **左格**：地圖式示意——左邊三個倉庫（圓，面積 ∝ 10、20、30），右邊三家店（方，面積 ∝ 25、15、20），中間畫箭頭，箭頭粗細 ∝ $\pi_{ij}$，用最佳解的箭頭（有的倉庫拆送兩家）；旁邊標每條箭頭的 $c_{ij}$。
- **右格**：$3\times3$ 的運輸表 $\pi_{ij}$，右側列和 $a_i$、下方欄和 $b_j$，格子的底色深淺 ∝ $\pi_{ij}$，每格右上角小字 $c_{ij}$；表下方寫「總油錢 $=\sum\pi_{ij}c_{ij}$」與最佳值。
- **中間一列**：兩張小表對照——「Monge：每列只有一格」（畫一張不可能湊出欄和的表，打叉）與「Kantorovich：允許拆」（打勾）。
- **圖說**：「一個運輸方案就是一張兩邊都對得上的表——一個 coupling。Monge 要求每列只有一格不為零，不一定填得出來；Kantorovich 允許拆貨，永遠有解。」
</VizPlan>
...
<VizPlan id="m6-0-ot" title="填一張運輸表：拖動需求、看最佳解怎麼拆" type="interactive">
- **位置**：本節之後。**檔名**：`m6-optimal-transport/m6-0-1.html`。
- **版面**：左格地圖（三倉庫、三店、箭頭粗細 ∝ $\pi_{ij}$）；右格 $3\times3$ 運輸表（可手動填數）與列和／欄和的檢查燈（對得上亮綠、對不上亮紅），下方顯示總油錢與「最佳解」按鈕（呼叫內建的 LP 求解）。
- **控制項**：三支滑桿改各店需求 $b_j$（總和自動守恆），三支滑桿改各倉庫供給；點任一格可改油錢 $c_{ij}$。
- **行為**：手填時檢查燈即時反應；按「最佳解」後表格填入 LP 的解、用色塊標出非零格、旁邊顯示「非零格數 $\le m+n-1$」；把 $a,b$ 拖成全相等（例如各 20）時，最佳解變成排列矩陣，畫面標「assignment：不用拆」。開關「獨立 coupling」顯示 $a_ib_j/\!\sum a$ 那張表與它的成本作對照。
- **第二個面板**：「Monge 可行嗎？」——對當前的 $a,b$ 枚舉所有 $3^3=27$ 種映射，列出合法的（若有）與它們的成本；多數 $a,b$ 下顯示「無合法映射」。
- **Hover** 任一格：白話「從倉庫 $i$ 送 $\pi_{ij}$ 箱到店 $j$，每箱 $c_{ij}$ 元，這條路線花 $\pi_{ij}c_{ij}$」。
- **底部觀察句**：「觀察：一個運輸方案就是一張兩邊都對得上的表。允許拆貨永遠填得出來，最佳解最多用 $m+n-1$ 條路線；供需相等時最佳解自動不拆。」
- **匯出 PNG**：左右併成 16:9，含當前 $a,b,c$ 與最佳成本。
</VizPlan>
...
<Bridge to="math-w2">
本篇把「搬貨最省油」寫成一張兩邊都對得上的運輸表——coupling——並看到 Kantorovich 的問法永遠有解、是線性規劃、最佳解落在少數幾條路線上。有了「最省的搬法」，就可以反過來問：兩個分佈「差多少」，能不能用「把一個搬成另一個最少要花多少」來量？下一篇把這個量定義出來，並看它和其他「差多少」的量哪裡不同。
</Bridge>
