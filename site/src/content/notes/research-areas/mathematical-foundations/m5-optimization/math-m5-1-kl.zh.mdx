...
<Objectives>
- 寫出 $\mathrm{KL}(p\,\|\,q)=\sum_x p(x)\log\frac{p(x)}{q(x)}$，並用「照 $q$ 下注、天氣照 $p$ 出現、每天平均多輸的量」讀懂它。
- 用 Jensen 兩行證明 $\mathrm{KL}\ge0$、等號只在 $p=q$；並說出 cross-entropy $=$ entropy $+$ KL 這條分解。
- 對一個「用錯的模型做決策」的情境判斷該用 $\mathrm{KL}(p\|q)$ 還是 $\mathrm{KL}(q\|p)$、說出兩者各在哪種錯上罰得重（$q$ 給真實事件太低機率 vs $q$ 把機率浪費在不會發生的事上），並用模擬驗證。
</Objectives>
...
<VizPlan id="m5-1-two-lines" title="兩條財富曲線：縫隙以 KL 的速度張開" type="static">
- **檔名**：`m5-optimization/imgs/m5-1-1.svg`，16:9。
- **主圖**：橫軸天數 $0$–$365$，縱軸 $\log W$。兩條鋸齒狀的線共用同一串抖動（同一組天氣），上面那條標「照 $p$ 下注」，下面那條標「照 $q$ 下注」；兩條線的長期趨勢各畫一條虛直線，兩虛線之間在 $t=365$ 處畫一段垂直距離，標「$365\times\mathrm{KL}(p\,\|\,q)$」。
- **右上角小格**：$p$ 與 $q$ 兩張三格的機率表並排（例如 $p=(0.5,0.3,0.2)$、$q=(0.6,0.1,0.3)$），差異用色塊標出。
- **底部一行**：「每天多輸 $\mathbb E_p[\log(p/q)]$，一年累積 365 倍；賭場的賠率消掉了，只剩你的表與真相的距離。」
- **圖說**：「同一串天氣下，用錯表的人財富曲線斜率固定低一截。斜率差就是 KL divergence——不是一次性的罰金，是每天付的租金。」
</VizPlan>
...
<VizPlan id="m5-1-kl" title="拖動 q 的三格，看每天多輸多少、哪一格最貴" type="interactive">
- **位置**：本節之後。**檔名**：`m5-optimization/m5-1-1.html`。
- **版面**：左格是兩張三格機率表 $p$（固定或可調）與 $q$（三支連動的滑桿，總和保持 1），下方即時顯示 $\mathrm{KL}(p\|q)$、$\mathrm{KL}(q\|p)$、$H(p)$、$H(p,q)$ 四個數，並用三根帶正負號的柱子顯示 KL 的三個逐項貢獻 $p(x)\log\frac{p(x)}{q(x)}$。右格是 $\log$ 財富曲線：同一串模擬天氣下「照 $p$」與「照 $q$」兩條線，$t=365$ 處標縫隙。
- **行為**：把 $q$ 的某一格往 0 拖，該格的貢獻柱子急速升高、$\mathrm{KL}(p\|q)$ 飆向 $\infty$、右格的下面那條線垂直墜落——「低估會發生的事」；把某一格往上拖到遠超 $p(x)$，貢獻柱子變負但很小、KL 只微增——「高估不會發生的事很便宜」。按鈕「交換 $p,q$」：兩個 KL 的數字互換位置，柱子重新分配，讀者看到同一對表不同方向罰不同格。
- **開關「顯示 cross-entropy 分解」**：把右格的縫隙拆成兩段：$H(p)$（灰，逃不掉的）與 $\mathrm{KL}$（紅，用錯表多付的）。
- **Hover** 任一根貢獻柱子：顯示白話「你以為 $x$ 的機率是 $q(x)$，其實是 $p(x)$；每次它發生你多付 $\log(p/q)$，它一年發生約 $365p(x)$ 次」。
- **底部觀察句**：「觀察：每天多輸 $\mathrm{KL}(p\|q)$，只跟你的表和真相有關。低估會發生的事，代價趨近無限；高估不會發生的事，代價很小。換方向就換了罰哪種錯。」
- **匯出 PNG**：左右併成 16:9，含當前 $p,q$ 與四個數字。
</VizPlan>
