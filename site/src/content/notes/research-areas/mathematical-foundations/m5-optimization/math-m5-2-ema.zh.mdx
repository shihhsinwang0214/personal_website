...
<Objectives>
- 寫出 exponential moving average $m_t=\beta m_{t-1}+(1-\beta)x_t$，說出它等價於「對過去以幾何權重加權平均」、有效記憶長度約 $1/(1-\beta)$，並算出它對雜訊的壓縮與對移動目標的滯後。
- 說出當目標由估計者自己定義時（bootstrap），為什麼「全部一起不動」是一個平凡解，以及 stop-gradient 與慢速目標如何避開它。
- 對一個「追蹤會動的量」的情境判斷該用多大的 $\beta$、說出這個判斷依賴的假設（目標的速度、雜訊的大小、是否有 bootstrap），並用模擬驗證與破壞它。
</Objectives>
...
<VizPlan id="m5-2-bucket" title="漏水的桶：EMA 是幾何權重的加權平均" type="static">
- **檔名**：`m5-optimization/imgs/m5-2-1.svg`，16:9。
- **左格**：一排往回延伸的讀數 $x_t,x_{t-1},x_{t-2},\dots$，每個下面一根權重柱 $(1-\beta)\beta^k$，柱高幾何遞減，標 $\beta=0.9$；柱子上方一條淡線標「有效記憶 $\approx1/(1-\beta)=10$ 步」。旁邊對照一組「最近 10 個等權平均」的柱子（10 根等高、之後為零），標「硬截止」。
- **右格**：一桶水的示意，箭頭標「倒入 $(1-\beta)x_t$」與「漏掉 $(1-\beta)m_{t-1}$」。
- **底部一行**：「$m_t=(1-\beta)\sum_{k\ge0}\beta^kx_{t-k}$，權重和為 1」。
- **圖說**：「EMA 只存一個數，卻等於對整個過去做幾何權重的加權平均。$\beta$ 決定漏多快，也就是記多久。」
</VizPlan>
...
**$\beta$ 是「跟多緊」與「多平滑」之間唯一的旋鈕。** 狗跑得快（$v$ 大）就該把 $\beta$ 調小、記得短；雜訊大（$\sigma$ 大）就該調大、記得長。最佳的 $\beta$ 讓兩項加起來最小——它有 closed form，但更重要的是它**由 $v$ 與 $\sigma$ 的比值決定**，不是一個放諸四海皆準的常數。
...
<VizPlan id="m5-2-ema" title="拖動 β：跟多緊、多平滑，以及追自己的影子" type="interactive">
- **位置**：本節之後。**檔名**：`m5-optimization/m5-2-1.html`。
- **版面**：上格是公園俯視：狗的真實軌跡（細線）、帶雜訊的讀數（灰點）、EMA 估計（粗線）逐秒前進（單趟 Play，不循環）；右側即時顯示實測滯後、實測雜訊標準差、與理論值 $v\beta/(1-\beta)$、$\sigma\sqrt{(1-\beta)/(1+\beta)}$ 並排。下格是「總誤差 vs $\beta$」的曲線，標出當前 $\beta$ 的位置與最小值。
- **控制項**：滑桿 $\beta$（$0$–$0.999$，對數間隔）；滑桿「狗速 $v$」與「雜訊 $\sigma$」（拖動時下格曲線的最低點跟著移動——讀者看到最佳 $\beta$ 由 $v/\sigma$ 決定）；按鈕「急轉彎」（$t=100$ 時速度反向，估計曲線在轉彎處拖出一段明顯的落後，畫面標「等速假設失效，約 $1/(1-\beta)$ 步後追回」）；開關「Adam 式 bias correction」（顯示前幾步 $m_t/(1-\beta^t)$ 與 $m_t$ 的差）。
- **第二個面板「追自己的影子」**：一個一維的 $\theta$ 與它的目標；三個模式切換：(a) 目標 $=\theta$ 本身、無 stop-gradient → $\theta$ 不動、loss 顯示 0、畫面標「平凡解」；(b) 加 stop-gradient、目標 $=$ 當前 $\theta$ 的某個變換 → $\theta$ 每步跳動、震盪；(c) 加 stop-gradient + EMA 目標（$\beta$ 滑桿）→ $\theta$ 平穩收斂。三條 $\theta$ 隨步數的曲線疊在一起。
- **底部觀察句**：「觀察：EMA 一個係數換兩個代價——$\beta$ 大壓雜訊、$\beta$ 小跟得緊，最佳值由 $v/\sigma$ 決定。當目標是自己定義的，站著不動也算追到；stop-gradient 切斷那條捷徑，EMA 目標讓自己變成一隻跑得很慢的狗。」
- **匯出 PNG**：兩個面板併成 16:9，含當前 $\beta$、$v$、$\sigma$ 與數值。
</VizPlan>
...
本篇把「追一個會動的目標」寫成 EMA：一個係數換兩個代價（滯後 $v\beta/(1-\beta)$、雜訊 $\sigma\sqrt{(1-\beta)/(1+\beta)}$），並看到當目標是自己定義的時候，stop-gradient 與慢速目標如何避開「大家一起不動」的平凡解。下一篇換一種控制：你手上有一台把亂數變成產品的機器，只看得到它吐出來的樣本，卻想調整它讓產品的**分佈**接近某個目標——要對什麼微分、梯度長什麼樣？
</Bridge>
