...
工廠裡有一台機器：每次按下按鈕，它內部擲一把骰子（亂數 $z$），然後依照參數 $\theta$ 把骰子的結果加工成一個產品 $x=G_\theta(z)$。你看不到骰子、看不到內部，只看得到吐出來的產品。

客戶給了你一個目標：產品的**分佈**要像某個規格 $p^*$——不是每個產品都一樣，而是整體的散佈要對。你可以調整機器的參數 $\theta$。這些參數應該往哪個方向更新？

先用直覺回答：你會怎麼做？拿一批產品、跟規格比、然後怎麼把「比出來的差距」變成「參數該更新的方向」？寫下你有多確定。

課堂上的回答通常是「多抽一些，看哪邊多了就往反方向調」。這是對的方向，但它說不出**每個參數該更新多少**——尤其當參數很多（$\theta$ 是幾百萬維）時，「哪邊多了」根本無法逐一看。我們需要的是一個**梯度**：分佈的差距對每個參數的偏導數。
...
![Generator gradient：score 差乘上 generator 靈敏度](./imgs/m5-3-1.svg)
...
<Explanation>
reparameterization 的核心是「讓參數改變一點、每個樣本連續地動一點」；離散輸出沒有「動一點」。$\arg\max$ 的梯度處處為零（跳躍點除外），autodiff 會給你零梯度。換距離不解決可微性的問題。score-function estimator 不需要 $G$ 可微，但變異數大——這是它與 reparameterization 之間永遠的取捨。
</Explanation>
</QuizBlock>
...
本篇把「抽樣」拆成「固定骰子 + 可微的機器」，於是 KL 對機器參數的梯度可以算：每個樣本處兩個 score 的差、乘靈敏度、取平均；那個期望為零的項留著只會增加雜訊。到這裡 M5 的四個工具——Jensen、KL、EMA、對 generator 微分——都建好了。接下來的單元 換一種「距離」的想法：不問「兩個分佈在每一點差多少」，而問「把一堆貨從這個擺法搬成那個擺法，最少要多少油」。
</Bridge>
