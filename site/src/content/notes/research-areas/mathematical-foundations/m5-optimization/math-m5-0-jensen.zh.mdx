...
<Objectives>
- 說出 convex function 的定義（弦在曲線上方），並用它寫出 Jensen 不等式 $\varphi(\mathbb E[X])\le\mathbb E[\varphi(X)]$ 與等號成立的條件。
- 用「損失的平均 vs 平均的損失」解釋為什麼兩邊會差、差距由什麼決定（$\varphi$ 的彎度 $\times$ $X$ 的散佈）。
- 對一個生活情境（保險、分散投資、排隊）判斷該把 Jensen 用在哪個方向、說出它依賴的假設（$\varphi$ 真的 convex、期望存在），並用模擬驗證與破壞它。
</Objectives>
...
<VizPlan id="m5-0-chord" title="弦在曲線上方：損失的平均 vs 平均的損失" type="static">
- **檔名**：`m5-optimization/imgs/m5-0-1.svg`，16:9。
- **左格**：一條往上彎的曲線 $\varphi$（痛苦 vs 損失），標兩個端點 $x=0$ 與 $x=200{,}000$，畫弦；在 $x=\mathbb E[X]=4{,}000$ 處畫一條垂直線，與曲線交於 $\varphi(\mathbb E[X])$（標「確定損失 4,000 的痛苦」）、與弦交於 $\mathbb E[\varphi(X)]$（標「不買保險的期望痛苦」）；兩點之間的紅色線段標「Jensen gap」。再在曲線上標 $x=6{,}000$ 的點 $\varphi(6{,}000)$，用虛線顯示它低於弦上的點——「多付 2,000 仍划算」。
- **右格**：同一條曲線，但 $X$ 換成保險公司的「十萬人平均損失」：兩個端點擠在 $4{,}000$ 附近很窄的區間裡，弦幾乎貼著曲線，Jensen gap 小到看不見；小字「$X$ 幾乎不隨機 → 弦與曲線重合 → 不怕」。
- **底部一行**：「gap $\approx\tfrac12\varphi''\cdot\mathrm{Var}(X)$：彎度 $\times$ 散佈」。
- **圖說**：「convex 的曲線，弦永遠在上方。$\mathbb E[\varphi(X)]$ 是弦上的點、$\varphi(\mathbb E[X])$ 是曲線上的點，兩者的差由彎度與 $X$ 的散佈決定。你怕、保險公司不怕，差在 $X$ 的散佈。」
</VizPlan>
...
<VizPlan id="m5-0-jensen" title="拖動 φ 的彎度與 X 的散佈，看 Jensen gap" type="interactive">
- **位置**：本節之後。**檔名**：`m5-optimization/m5-0-1.html`。
- **版面**：左格是 $\varphi$ 的曲線與弦；$X$ 的分佈畫成橫軸上的若干根柱子（高度 = 機率），$\mathbb E[X]$ 處一條垂直線，線上標出 $\varphi(\mathbb E[X])$（曲線上）與 $\mathbb E[\varphi(X)]$（弦／加權平均，用一個空心點）。兩點間的紅色線段即時顯示 gap 數值，旁邊顯示 $\tfrac12\varphi''(m)\mathrm{Var}(X)$ 的估計值以資對照。右格是「保費 vs 買不買」的一條橫軸：標 $\mathbb E[X]$、以及「使 $\varphi(\text{保費})=\mathbb E[\varphi(X)]$ 的最高可接受保費」，兩者之間的區間著色標「理性溢價」。
- **控制項**：滑桿「彎度」（$\varphi(x)=x+\kappa x^2$ 的 $\kappa$，可以拉到負值——變成 concave）；滑桿「散佈」：把 $X$ 的兩個取值拉開或收攏（保持 $\mathbb E[X]$ 不變）；滑桿「保單池大小 $n$」（1 到 $10^5$，對數）：$X$ 換成 $n$ 個獨立損失的平均，柱子向 $\mathbb E[X]$ 收攏。
- **行為**：$\kappa\to0$ 時弦與曲線重合、gap 歸零、右格的理性溢價區間消失；$\kappa<0$ 時空心點落到曲線**下方**、gap 變號、右格顯示「不等式反向：這個人寧願賭」；$n$ 增大時 gap 隨 $1/n$ 縮小，畫面標「保險公司的視角」。
- **Hover** 曲線任一點：顯示該點的切線（支撐線），以及「整條曲線都在切線上方」的高亮——證明的核心畫面。
- **底部觀察句**：「觀察：convex 時弦在曲線上方，$\mathbb E[\varphi(X)]\ge\varphi(\mathbb E[X])$；gap 約等於彎度乘散佈。$X$ 不隨機時 gap 為零——這就是保險公司不怕的原因。彎度反向，結論也反向。」
- **匯出 PNG**：左右併成 16:9，含當前 $\kappa$、散佈、$n$ 與 gap 數值。
</VizPlan>
...
<Bridge to="math-kl">
本篇把「怕」寫成一條往上彎的曲線，「弦在曲線上方」就是 Jensen 不等式：$\varphi(\mathbb E[X])\le\mathbb E[\varphi(X)]$，gap 由彎度乘散佈決定，$X$ 不隨機時等號成立。下一篇把 Jensen 用在 $\varphi=-\log$ 上：用一張錯的機率表下注一年，平均會多輸多少——那個「多輸的量」有一個名字，而它永遠非負的證明只有兩行。
</Bridge>
