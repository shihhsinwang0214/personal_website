...
- **說出 Euler–Maruyama 的 strong order 是 $1/2$、weak order 是 $1$。** 解釋為什麼同一個方法會有兩個不同的速率。
...
![兩把尺：逐路徑比，還是比直方圖](./imgs/m3-4-1.svg)
...
本篇把「模擬準不準」拆成兩把尺：strong 逐路徑比、weak 只比分佈；Euler–Maruyama 每步漏掉的首項是零均值的隨機量，所以 strong 只有 $\tfrac12$ 階、weak 有 $1$ 階——要的是樣本就看 weak。到這裡，連續狀態的隨機動力學（random walk → SDE → Fokker–Planck → Langevin → 誤差）走完了。接下來的單元 把狀態空間換成有限個格子：明天的天氣只跟今天有關，矩陣乘法就能算出一百天後晴天的比例。
</Bridge>
