...
- **對「一群粒子在有噪聲的系統裡分佈怎麼演化」這類情境，判斷用 Fokker–Planck 是否恰當、說出它依賴的假設（每步噪聲零均值、variance 有限、高階矩不太大；$g$ 不依賴 $x$）。** 用直方圖對照 PDE 的解驗證與破壞它。
</Objectives>
...
![搬與攤：Fokker–Planck 的兩項各做什麼](./imgs/m3-2-1.svg)
...
本篇把 SDE 的分佈演化寫成 Fokker–Planck 方程式——drift 搬、噪聲攤——並學會用「代入驗證」確認一個候選解。搬與攤如果方向相反：drift 一直把葉子往某處拉、噪聲一直把它們攤開，久了會停在一個平衡。接著把河換成山谷，問「很久以後大家在哪」。
</Bridge>
