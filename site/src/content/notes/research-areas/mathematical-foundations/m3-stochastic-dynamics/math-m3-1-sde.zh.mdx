...
<Objectives>
- 把 $dx=f(x,t)\,dt+g(t)\,dW_t$ 讀成「每一小步 $h$：確定地走 $f h$，再加一個 $g\sqrt h\,\varepsilon$ 的 Gaussian」，並說出 $f$（drift）與 $g$（diffusion coefficient）各控制什麼。
- 寫出 Euler–Maruyama 法，並說明為什麼噪聲項是 $\sqrt h$ 而不是 $h$、為什麼 SDE 的「解」是一個分佈而不是一條路。
- 對「有亂流的河裡葉子會漂到哪」這類情境，判斷用 SDE 模擬是否恰當、說出它依賴的假設（噪聲是 Brownian 增量、$g$ 不依賴 $x$），並用模擬驗證與破壞它。
</Objectives>
...
<VizPlan id="m3-1-fan" title="一條路變成一把扇子" type="static">
- **檔名**：`m3-stochastic-dynamics/imgs/m3-1-1.svg`，16:9。
- **版面**：俯視的河，主流往右。左半：$g=0$，一片葉子的 ODE 軌跡（一條平滑曲線，沿 $u=(0.5,\,0.1\cos(x/5))$ 微微擺動）。右半：同一個 drift 加 $g=0.3$，畫 100 條 Euler–Maruyama 路徑（細線半透明）從同一起點出發，疊上中心線（原 ODE 軌跡，粗線）與 $\pm g\sqrt t$、$\pm2g\sqrt t$ 的包絡（虛線）。
- 在 $t=60$ 處畫一條垂直的截線，旁邊小圖是該時刻橫向位置的直方圖疊 Gaussian，標「均值 ≈ 30 m，標準差 ≈ $0.3\sqrt{60}\approx2.3$ m」。
- **圖說**：「SDE 的解不是一條路，是一把扇子。中心線由 drift $f$ 決定，扇子寬度由 $g\sqrt t$ 決定。」
</VizPlan>

## 寫成數學：讀法、模擬、以及為什麼是 $\sqrt h$
...
<VizPlan id="m3-1-em" title="Euler–Maruyama：對的尺度、錯的尺度、相關的噪聲" type="interactive">
- **位置**：本節之後。**檔名**：`m3-stochastic-dynamics/m3-1-1.html`。
- **版面**：左格俯視的河，畫 200 條 Euler–Maruyama 路徑成扇形，疊 ODE 中心線與 $\pm g\sqrt t$ 包絡；右格是 $t=T$ 的橫向位置直方圖疊理論 Gaussian $\mathcal N(x_0+cT,g^2T)$，右上角顯示實測均值、標準差與理論值。
- **控制項**：滑桿「步長 $h$」（$3,\,1,\,0.3,\,0.1,\,0.03$ 秒）；滑桿「主流 $c$」與「亂流強度 $g$」；三選一「噪聲尺度」：$\sqrt h\,\varepsilon$（正確）、$h\,\varepsilon$（錯誤）、$\varepsilon$ 不縮放（錯誤）；滑桿「噪聲相關 $\rho$」（$0$–$0.95$，以 $h$ 為單位的相鄰步相關）；下拉「drift」：等速、往中線拉回的 $-0.1\,y$、擺動的 $0.1\cos(x/5)$。按鈕「重抽」。
- **行為**：正確尺度下拖 $h$，扇形寬度與直方圖**不動**，只有路徑變得更細碎；$h\varepsilon$ 時 $h$ 越小扇子越窄、直方圖縮成一根柱；不縮放時扇子爆開。拖 $\rho>0$：直方圖變寬、實測標準差高於理論值，且縮小 $h$ 時倍數改變，畫面標「噪聲不是 Brownian 增量，讀法失效」。
- **Hover** 左格任一路徑：把它的路拆成兩條顯示——drift 累積的部分（平滑）與噪聲累積的部分（random walk），讓讀者看到「SDE = ODE + random walk」。
- **底部觀察句**：「觀察：每一步走 $fh$、再加 $g\sqrt h\,\varepsilon$，就是 SDE 的全部；$\sqrt h$ 讓亂流的總效果與步長無關。寫成 $h\varepsilon$ 噪聲會隨步長消失；噪聲前後相關時扇子比理論寬，且寬多少取決於 $h$。」
- **匯出 PNG**：兩格併成 16:9，含當前 $h$、$c$、$g$、尺度選項與 $\rho$。
</VizPlan>
...
本篇把 SDE 讀成「每一小步走 $fh$、加 $g\sqrt h\,\varepsilon$」，看到它的解是一把扇子而不是一條路，並用 variance 與 $h$ 無關來檢驗 $\sqrt h$。一片葉子的分佈我們用模擬得到；下一篇像 <Ref to="math-continuity-equation"/> 那樣，不追葉子，直接寫下這個分佈自己滿足的方程式。
</Bridge>
