...
<Objectives>
- 把「一個會動的觀測者讀到的變化」拆成 $\partial_t f+\nabla f\cdot\dot x$ 兩項，並說出每一項各自是什麼、拆開需要什麼假設。
- 說出 Leibniz 法則（微分穿過積分）在什麼條件下成立，以及它和全導數共用的那個「微分可以逐項拆開」的邏輯。
- 對「讀數上升所以水變熱了」這個判斷，說出它漏了什麼、要補哪個量測才能拆開；並用 `torch.func.jvp` 三行算出全導數，說出為什麼不需要整個 Jacobian。
</Objectives>
...
<VizPlan id="m0-3-two-sources" title="讀數的變化來自兩個來源" type="static">
- **檔名**：`m0-calculus-toolkit/imgs/m0-3-1.svg`（16:9，左右兩格）。
- **左格**：河段俯視圖，用顏色深淺畫溫度場 $T(x,t)$（下游一塊暖區）。溫度計在時間 $t$ 與 $t+\Delta t$ 的兩個位置畫出來，中間一支箭頭標 $\dot x$（漂速）。在**同一個**位置 $x(t)$ 畫一個小時鐘圖示與向上的小箭頭，標「這裡的水在變熱：$\partial_t T$」；沿漂移方向畫溫度色階的變化，標「漂到更暖處：$\nabla T\cdot\dot x$」。
- **右格**：讀數對時間的折線 $g(t)$，在 $t$ 附近把斜率拆成兩段疊起來的柱狀（一段標 $\partial_t T$、一段標 $\nabla T\cdot\dot x$），總高度是 $g'(t)$。
- **圖說**：「溫度計讀到的變化率是兩個來源之和。單看讀數拆不開；要拆開，至少要另外量到其中一個。」
</VizPlan>
...
這就是 Leibniz 法則：**微分可以穿過積分符號，變成對被積函數的 partial derivative。** 它成立的條件是 $\partial_t T$ 存在，並且 $|w(x)\,\partial_t T(x,t)|$ 能被一個不依賴 $t$、可積的函數壓住（技術名詞是 dominated convergence；直覺是「沒有任何一支溫度計的變化率大到壞掉整個平均」）。一般形式是 $\frac{d}{dy}\int f(x,y)\,dx=\int\partial_y f(x,y)\,dx$——變數叫什麼不重要，重要的是被微分的變數**不是**積分變數，而且積分範圍不隨它改變。這條規則在之後很多推導裡是關鍵的一步，值得在這裡先站穩。
...
<VizPlan id="m0-3-jvp" title="拆開兩個來源：釘一支溫度計、加一條支流、比較 JVP 與 Jacobian 的成本" type="interactive">
- **位置**：本節之後。**檔名**：`site/public/notes/research_areas/mathematical-foundations/m0-calculus-toolkit/m0-3-2.html`。這是本篇的核心互動。
- **版面**：上半是一維河道（橫軸位置 0–600 m），溫度用色帶畫出，浮球沿河漂；下半是三條隨時間的曲線：浮球讀數 $g(t)$、釘住溫度計的讀數、以及「兩者之差」。右側一個小面板即時顯示 $\partial_t T$、$\nabla T\cdot\dot x$、$g'(t)$ 三個數字與一個疊起來的柱狀（對應靜態圖右格）。
- **控制項**：滑桿「整體升溫率」$\partial_t T$（$0$–$0.002$ °C/s）、滑桿「暖區強度」（$0$–$1$ °C）、滑桿「漂速」$\dot x$（$0$–$2$ m/s）；按鈕「放進河裡」開始十分鐘的模擬（不自動循環）；開關「釘一支溫度計在起點」顯示第二條曲線；開關「加一條支流」：在 $x=400$ m 插入一個溫度跳躍 $+1.5$ °C（寬度 1 m），浮球漂過時讀數會瞬間跳、面板上的 $\nabla T$ 顯示為「未定義」並亮紅。
- **Hover** 下半任一時刻顯示該時刻的三個量，並用一句話寫出「此刻讀數變化中有 $a\%$ 來自升溫、$b\%$ 來自漂移」。
- **右下角一個成本表**（靜態文字）：「算整個 Jacobian：$d$ 次傳播、$d^2$ 記憶體；算 JVP：約 2–3 次前向的成本、$d$ 記憶體」，旁邊一個滑桿 $d$ 從 $10$ 到 $10^5$，表格的數字跟著變。
- **底部觀察句**：「觀察：浮球讀數的斜率＝釘住溫度計的斜率＋漂移項，兩條曲線一起看才能拆開；只有浮球那一條，任何拆法都說得通。漂過支流時 $T$ 不 differentiable，這條公式在那一刻不適用。」
- **匯出 PNG**：上下兩半併成 16:9，含當前參數。
</VizPlan>
...
本篇把「會動的觀測者讀到的變化」拆成 $\partial_t f+\nabla f\cdot\dot x$，把「對很多項的和微分」寫成 Leibniz 法則，並用 JVP 在程式裡一次算出全導數而不碰 Jacobian。到這裡，微積分工具箱的四件工具——gradient、divergence、Taylor、全導數——都備齊了。下一個 class 換一種不確定性：不是「函數在變」，而是「量到的數字本身有雜訊」。第一個問題很日常：兩台體重計讀數不同，該怎麼合併？
</Bridge>
