...
<Objectives>
- 把「腳下的傾斜」寫成 directional derivative，並說出為什麼兩個垂直方向的讀數就決定了所有方向的傾斜。
- 寫出 gradient 的定義，證明最陡的方向就是 gradient 的方向，並說出 $\nabla\log f$ 為什麼是「相對變化率」。
- 對「只憑腳下傾斜往最陡處走」這個下山策略，判斷它在什麼假設下有效、何時失效，並用一個可以調步長的 demo 驗證。
</Objectives>
...
<VizPlan id="m0-0-tilt" title="一張桌面的傾斜：兩個讀數決定所有方向" type="static">
- **檔名**：`m0-calculus-toolkit/imgs/m0-0-1.svg`（16:9，三格並排）。
- **左格**：俯視一小塊斜的桌面（畫成一個方格），中央一顆彈珠、一支粗箭頭標示它會滾的方向；沿東西向與南北向各畫一支細箭頭，旁邊標上兩個讀數「東西向斜率 $-0.3$」「南北向斜率 $-0.4$」。
- **中格**：同一塊桌面，畫出八個方向的斜率長條（像羅盤），每根長條的長度是該方向的斜率；最長的那根對齊彈珠滾的方向，垂直方向的長條為零。標「兩個讀數 → 全部八根都算得出來」。
- **右格**：側視同一塊桌面，畫出「沿粗箭頭方向」的剖面（最陡）與「沿垂直方向」的剖面（水平），標上陡度 $0.5$（＝$\sqrt{0.3^2+0.4^2}$）。
- **圖說**：「腳下那一小塊地面可以當成平面。平面的傾斜只有一個方向、一個陡度；量兩個垂直方向就夠了。」
</VizPlan>
...
一個值得記住的例子：$f(x)=\exp\!\big(-\|x-\mu\|^2/(2\sigma^2)\big)$（一個鐘形，中心 $\mu$、寬度 $\sigma$）。直接算：
...
<VizPlan id="m0-0-descent" title="霧中下山：改步長、換地形，看策略何時失效" type="interactive">
- **位置**：本節。**檔名**：`site/public/notes/research_areas/mathematical-foundations/m0-calculus-toolkit/m0-0-2.html`。這是本篇的核心互動。
- **版面**：左格是俯視的等高線地形（canvas，約 60% 寬），右格是「高度 vs 步數」的折線圖。地形可從下拉選單切換：**碗**（一個低點，$h=x^2+2y^2$）、**雙窪地**（兩個低點、中間一道鞍）、**稜線**（含一條 $|x|$ 型的不可微折線）。
- **控制項**：滑桿「步長 $\eta$」（$0.01$–$1.5$）；按鈕「走一步」「走 50 步」「重置」；點擊地形任一處設定起點。開關「顯示 $-\nabla h$ 箭頭場」：在網格上畫出每一點的 $-\nabla h$（箭頭長度 ∝ 陡度）。開關「霧」：只顯示走過的軌跡與腳下一小圈的等高線，模擬看不見全局。
- **每一步的行為**：從當前位置 $p$ 走到 $p-\eta\,\nabla h(p)$；軌跡畫成連線，右格同步畫出 $h$ 的值。Hover 軌跡上任一點顯示該處的 $\nabla h$ 兩個分量與 $\|\nabla h\|$。
- **預期看到的事**：碗＋小步長：高度單調下降、收斂到底部。碗＋$\eta$ 過大（在 $h=x^2+2y^2$ 上 $\eta>0.5$，因為 $y$ 方向每步乘上 $1-4\eta$）：沿 $y$ 方向來回跨過谷底、高度反而上升——右格折線震盪發散。雙窪地：從不同起點出發會停在不同的窪地，右格顯示「停在 $\|\nabla h\|\approx0$，但不是全域最低」。稜線：軌跡在折線兩側來回抖動、無法沿著稜線走下去。
- **底部觀察句**：「觀察：$-\nabla h$ 是腳下那一小塊的最陡方向，每一步都對；但它是局部資訊——步長太大會跨過谷底，地形有多個低點時停下來的地方未必最低，稜線上根本沒有 gradient。」
- **匯出 PNG**：兩格併成 16:9，含當前地形、步長、軌跡。
- **註記**：真實的最佳化問題常在幾百萬維，畫二維只是為了看得見；「兩個讀數決定 gradient」在 $d$ 維變成「$d$ 個 partial derivative 決定 gradient」。
</VizPlan>
...
本篇把「腳下的傾斜」整理成一個向量 $\nabla h$：兩個讀數決定所有方向、最陡方向是 $-\nabla h$、而 $\nabla\log f$ 是不在乎整體倍率的相對變化率。同時看到它是局部資訊——步長與多個低點是它的兩個失效點。下一篇換一個問題：不是「往哪走」，而是「一個區域裡的東西在變多還是變少」——那需要另一種微分。
</Bridge>
