...
![曲線與切線在 h、2h、3h 處的偏差依一、四、九成長。](./imgs/m0-2-1.svg)
...
前兩項就是「假設直線」；$R_1$ 是直線與真實的差，叫 **remainder**（餘項）。它的形狀要看的是：一個 $h^2/2$，乘上**這段時間內某一刻的二階導數**——對軌跡來說就是加速度。所以直線假設的誤差 $\le\frac{h^2}{2}\max|f''|$：**時間加倍、上界四倍；加速度加倍、上界兩倍。**
...
本篇把「假設直走」寫成一階 Taylor，看到它的誤差是 $\frac{h^2}{2}\|\ddot x\|$——步長平方乘加速度——並確認這個量級只在步長夠小、軌跡夠 smooth 時成立。這只是**一步**的誤差；很多步接連下去會累積成什麼，是 <Ref to="math-euler-error"/> 要處理的。接著先補最後一件微分工具：一個量同時因為「時間在走」與「位置在動」而改變時，怎麼把兩種來源拆開。
</Bridge>
