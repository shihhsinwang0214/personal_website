...
<Objectives>
- **認出一條 chain 裡的 absorbing state（$P(x,x)=1$ 的格子）。** 說出它讓 stationary distribution 變得平凡、讓「吸收時間」成為真正要算的量。
...
<Bridge to="math-ctmc">
這一篇認出「進去就出不來」的格子，看到它讓長期比例變得平凡、讓「多久被吸收」成為主角，並用 first-step analysis 一行解出期望。到目前為止時間都是一回合一回合走的。接著處理「隨時都可能發生」的事——電話隨時進來、隨時結束——那時 transition matrix 要換成另一個物件。
</Bridge>
