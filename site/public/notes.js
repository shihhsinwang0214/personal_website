...
const notesData = {
    "themes": [
        {
            group: { en: "Flow-Based Generative Models / From Noise to Data", zh: "Flow-Based Generative Models / From Noise to Data" },
            title: { en: "The Probability Flow ODE: Bridging Score and Flow", zh: "Probability Flow ODE：Score 和 Flow 的橋" },
            route: { en: "notes/n2d-probability-flow-ode", zh: "zh/notes/n2d-probability-flow-ode" }
        },
        {
            group: { en: "Flow-Based Generative Models / From Noise to Data", zh: "Flow-Based Generative Models / From Noise to Data" },
            title: { en: "Velocity as a Regression Target: What Flow Matching Learns", zh: "Velocity 當成回歸目標：Flow Matching 在學什麼" },
            route: { en: "notes/n2d-velocity-regression", zh: "zh/notes/n2d-velocity-regression" }
        }
    ],
    "skills": [
        {
...
const homeTranslations = {
    ...
    "upd-1": { en: "Migrated the flow-based generative modeling notes to From Noise to Data.", zh: "將 flow-based generative modeling 筆記整理到 From Noise to Data。" },
...
function loadCategory(categoryId, event, specificIndex = null) {
    ...
    notesData[categoryId].forEach((note, index) => {
        ...
        if (note.route) {
            btn.setAttribute("data-route", note.route[currentLang]);
            btn.onclick = () => {
                window.location.href = "/personal_website/" + note.route[currentLang];
            };
        } else {
    ...
    if (btnToClick && btnToClick.getAttribute("data-route")) {
        window.location.href = "/personal_website/" + btnToClick.getAttribute("data-route");
    } else if (btnToClick) {
